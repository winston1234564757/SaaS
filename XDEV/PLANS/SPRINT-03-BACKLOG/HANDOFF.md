...
## 🔧 HOTFIX — Flash deals: "Сповіщено 0 клієнтів" — ВИПРАВЛЕНО (commit: 9f5a835)

**Дата:** 2026-06-11 | **Deploy:** `bookit-five-psi.vercel.app`

**Баг 1 (код):** `actions.ts` передавав 2 параметри в RPC замість 3. PostgreSQL кидав "wrong number of arguments" — помилка мовчки ковталась → `eligibleRows = null` → 0 клієнтів.

**Баг 2 (логіка):** RPC вимагав completed booking саме *цієї послуги* 14+ днів тому OR майбутній запис. `p_slot_timestamp` у сигнатурі але ніде не використовувався. Майже всі клієнти виключались.

**Міграція:** `simplify_flash_deal_eligible_clients` — спрощено до `p_master_id` одного параметра, повертає `client_name` разом з `client_id`, фільтрує лише клієнтів з записом у найближчі 3 дні.

**UI:** результат тепер показує список імен клієнтів (як у broadcast) замість числа.

**Файли:** `flash/actions.ts`, `FlashDealPage.tsx`
