...
## ✅ БЕКЛОГ ЗАКРИТО: 13/19 ✅ · 5 ↩️ · 1 ⏸ (EXPL-01, after-launch)
...
| `OPT-DB-03` | scope-розділення finances/stock (**баг**) | ✅ | `51555a37` |
| `OPT-DB-04` | set-diff → SQL (**баг коректності**) | ✅ | `51555a37` |
| `OPT-ASSET-03` | recharts defer | ↩️ скасовано | — |
| `OPT-DB-01` / `DB-02` / `DB-05` | перф-задачі Фази DB | ↩️ скасовано (заміри прода) | — |
...
### ФАЗА DB — ЗАКРИТО (2026-07-12) · `51555a37`. Замір спростував 3 з 5, але знайшов 2 баги

**Замір прода ПЕРЕД кодом** (Management API, read-only):

| Таблиця | Рядків | Max на майстра |
|---|---|---|
| `bookings` | 283 | **244** |
| `orders` | 5 | 5 |
| bookings із товарами (2-й запит `DB-01`) | **13** | 13 |
| `client_master_relations` | 7 | **5** |
| `products` | 4 | 4 |

**↩️ Три задачі скасовано — передумова не тримається:**

- **`DB-01`** «full-scan усієї історії замовлень» = **5 замовлень + 13 бронювань**. Другий запит має `booking_products!inner`, тобто тягне лише бронювання з товарами. Продаж товарів — рідкісна подія (4 товари в системі).
- **`DB-02`** «refetch на кожен keystroke» — **ХИБНО. Дебаунс 300 мс уже стоїть** (`BroadcastEditor.tsx:145-153`), плюс React Query кешує кожен ключ 30 с. Клієнтів максимум 5 на майстра. Пагінація RPC = робота заради нуля.
  ⚠️ Тут я мало не помилився: спершу вирішив, що `getClientsForPicker`/`previewBroadcastRecipients` — мертвий код, бо грепнув `useClientsPicker` замість справжнього `useClientsForPicker`. Обидва хуки **живі** (`BroadcastEditor.tsx:125,127`). **Перевіряй точну назву символу, перш ніж оголошувати код мертвим.**
- **`DB-05`** «8 скан-проходів» — `get_finance_analytics` виконується **7.8 мс** на 283 бронюваннях. Переписувати робочий фінансовий RPC у CTE = ризик зламати цифри заради мілісекунд.

**🔴 Пастка заміру, на яку сам наступив:** перший вимір `get_analytics_extras(scope='all')` дав **158 мс** — і це виглядало як підтвердження P0. Це був **холодний старт**. Прогріті виміри: 18 мс. **Перший виклик у сесії міряє планування, а не роботу — завжди прогрівай.**

**✅ Два справжні баги, яких у беклозі не було** (обидва про коректність, не швидкість):

1. **`DB-03` — подвійне обчислення в SQL.** `IF p_is_pro AND p_scope IN ('all','finances','stock')` рахував **і** `finances`, **і** `stock_forecast` для **обох** scope. `StockTab` (scope='stock') дарма ганяв найважчу функцію аналітики. Доказ: обидва scope віддавали байт-у-байт однаковий payload (2618 Б), хоча кожен таб читає лише свою половину (`StockTab.tsx:78`, `FinancesTab.tsx:95,125`). → дві незалежні умови, `20260712000001`.
   **Прод після фіксу:** `stock` 9.7мс/2618Б → **3.6мс/723Б** · `finances` 7.2мс → **5.5мс**.
   ⚠️ **`p_scope text DEFAULT 'all'` треба зберігати 1:1** — `CREATE OR REPLACE` не вміє прибирати дефолти («cannot remove parameter defaults»). **Локальний прогін спіймав це до прода** — прод-інтроспекція через `pg_get_function_identity_arguments` дефолтів НЕ показує, потрібен `pg_get_function_arguments`.
   *Перф-частина `DB-03` (lazy per-tab) — ↩️: прогрітий `all` = 18 мс, економія була б ~6 мс і 2 КБ.*

2. **`DB-04` — тиха хибність.** `.limit(5000)` **без `ORDER BY`**: Postgres не гарантує, ЯКІ рядки поверне. Щойно в майстра стане >5000 історичних бронювань, частина телефонів випаде з `prevPhones` → **давній клієнт мовчки порахується як «новий»**. Помилки не буде — просто цифра на дашборді стане брехливою. Сьогодні 244 < 5000, тож не стріляє. → RPC `get_week_new_client_phones` (`NOT EXISTS`), `20260712000002`.
   **Бриф казав «повертати число, не масив» — живий код спростував:** `weekNewPhones` споживається (`StatsModals.tsx:93` через `Set<string>`). RPC віддає **самі нові телефони** — їх одиниці, замість 5000 історичних. Індекс не потрібен: покриває `idx_bookings_master_status_date`.

**✅ Верифіковано на живих даних** (edge-кейси засіяв сам, прибрав за собою — 9 рядків, 0 залишку):
- **паритет** старого JS-алгоритму і нового RPC — ідентичний, окрім свідомо виправленої вади: телефон `---` нормалізувався в `''` і рахувався **окремим «новим клієнтом»**, завищуючи цифру;
- нормалізація зливає формати з роздільниками (`067-555-55-55` = `0675555555` → не новий);
- історія лише зі **скасованих** не ховає клієнта (новий ✓); скасовані цього тижня не течуть у результат;
- **негативний IDOR-тест:** чужий майстер → `42501` на **обох** функціях; власник бачить своє. *(Перший прогін був недійсний — `SET LOCAL` поза транзакцією не застосовується, `auth.uid()` лишався NULL і guard просто не спрацьовував. Тест треба гнати в `BEGIN…COMMIT`.)*
- anon без `EXECUTE` · `tsc` 0 · `build` 54/54 · **1036 unit-тестів** · прод-стан перевірено запитами ПІСЛЯ apply.

**🔔 Тригери повернення до ↩️-задач:** `DB-01` — коли в майстра з'явиться >500 замовлень; `DB-02` — >300 клієнтів; `DB-05` — коли `get_finance_analytics` перевалить за ~200 мс. Перевіряти тим самим `count(*)`, не читанням коду.

**⚠️ Спірна нормалізація телефонів (поза скоупом, НЕ фіксив).** `replace(/\D/g,'')` не канонізує номер: `+380 (67) 111-11-11` → `380671111111`, а `0671111111` → `0671111111`. Той самий клієнт у двох форматах рахується як двоє. Це наскрізна властивість продукту (та сама `norm` у `StatsModals`), змінювати її треба окремою задачею з міграцією даних — не тихцем усередині перф-фіксу.
...
Аудит писався зі статичного читання. Жива перевірка спростувала або звузила його **десять разів**: `ASSET-03` ↩️ · `DB-07` 5→2 · `RND-05` 6→3 · `ASSET-02` 9→5 · `RND-06` ↩️ · `DB-08` (міграція зайва) · `RND-04` ч.2 ↩️ · **`DB-01` ↩️ · `DB-02` ↩️ · `DB-05` ↩️**. Наступна сесія має так само **перевіряти передумову перед виконанням**, а не виконувати беклог наосліп.

> **⭐ МЕТА-УРОК УСЬОГО БЕКЛОГУ.** Статичний аудит надійно каже, **що станеться при великому N**, і не каже **нічого** про те, **чи існує велике N**. Шість задач із дев'ятнадцяти описували проблеми, яких на живих даних немає. Той самий замір, що їх закрив, знайшов **два баги, яких у беклозі не було** — і обидва про **коректність**, не про швидкість (`.limit()` без `ORDER BY`; подвійне обчислення в scope). Перф-аудит шукав повільний код, а знайшов **брехливі цифри**. Тому: спершу `count(*)` на проді, і тільки потім — рішення.
...
### Фаза 2 (DB/RPC) — ✅ ЗАКРИТО 2026-07-12

Повний розбір — у секції «ФАЗА DB — ЗАКРИТО» вище. Коротко: `DB-03` і `DB-04` зроблено (обидва — **баги**, не перф), `DB-01`/`DB-02`/`DB-05` ↩️ скасовано за замірами прода. Тригери повернення записані там же.

> **Блокера «apply робить founder» більше нема:** прод-apply через Management API відпрацьований, реєстр чистий (`db push` = no-op). Рецепт apply + пастки PowerShell — у `TRANSITION_PROMPT §Прод-apply`.
