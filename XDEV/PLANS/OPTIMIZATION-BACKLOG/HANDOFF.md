...
## Стан: 11/19 ✅ · 2 ↩️ · 1 ⏸ · **лишилась ФАЗА DB (5 задач)**

> ✅ **Міграції `ASSET-02b` застосовано на прод** (2026-07-11) — Management API, обидві версії в реєстрі, стан перевірено запитами після застосування. Аплоад DM-вкладень на проді полагоджено (баг був у політиках БД, тож працює вже зі старим задеплоєним кодом).
> ✅ **Реєстр міграцій вирівняно** (2026-07-11/12, `862e278f` + `5dad6eaa`). `db push --dry-run` → чистий no-op; 185 спарено / 0 local-only / 0 orphans. Стара заборона «`db push` не запускати» **знята** — деплой-флоу лишається `vercel --prod` + Management API, але `db push` більше не міна. Розбір: `XDEV/AUDIT/REPO_PARITY.md §HALF #2 ЗАКРИТО`.
> ℹ️ `.env.production.local` → перейменовано в `.env.e2e-build.bak`, `npm run build` знову чесний прод-білд (`0098b30e`).
...
| `OPT-RND-04` ч.2 | віртуалізація `ChatMessageList` | ↩️ скасовано (заміри прода) | — |
...
### `OPT-RND-04` ч.2 — ↩️ СКАСОВАНО (2026-07-12). Проблеми не існує

**Замір на проді ПЕРЕД кодом** (Management API, read-only):

| Таблиця | Рядків усього | Розмов | Найдовша | Розмов > 100 |
|---|---|---|---|---|
| `direct_messages` | 2 | 2 | **1 повідомлення** | 0 |
| `support_messages` | 10 | 2 | **9 повідомлень** | 0 |

Віртуалізація ріже DOM/motion-ноди. Нод, які треба різати, **не існує** — і не існуватиме, доки *одна пара* майстер–клієнт не набере ~200+ повідомлень. Для booking-SaaS, де чат = «коли зможете?» / «завтра о 14», це роки. Ціна ж — висока вже зараз: власний скрол-контейнер (не window-scroll, тож референс ч.1 з `useWindowVirtualizer` не переноситься), reverse-scroll, стик-до-низу, вставка realtime-INSERT, змінна висота через вкладення, конфлікт із `popLayout`.

**Чесна рамка:** прод порожній не тому, що проблему вирішено, а тому що продукт ще майже не використовується — тож замір **не доводить**, що навантаження не прийде. Він доводить інше: **сьогодні воно нульове, і крива до нього довга й повільна.** Робити складний ризиковий рефактор під це — та сама пастка, що в `RND-06`.

**🐞 Що знайдено в коді попутно (НЕ полагоджено — свідомо, за рішенням founder):**
- `AnimatePresence mode="popLayout"` у `ChatMessageList.tsx:76` — **жоден `motion.div` не має `exit`.** `popLayout` тут не робить своєї роботи; єдине, що AnimatePresence реально дає, — `initial={false}`, який гасить entrance-анімацію всієї історії при відкритті чату.
- `layout` на **кожному** повідомленні (`:87`) — найдорожче тут. Framer міряє всі layout-елементи (before+after) при кожній зміні → O(N) reflow-вимірів на **кожне нове повідомлення**. Деградує **раніше**, ніж кількість DOM-нод стане проблемою. Причому список і так примусово скролиться в низ на кожну зміну `messages` (`:45-47`), тож сама layout-анімація майже невидима.
- `useDMChat.ts:30-34` і `useLiveChat.ts:38-42` — `select` без `.limit`, тягнеться **вся** історія розмови.

**🔔 ТРИГЕР ПОВЕРНЕННЯ.** Відкривати задачу заново, коли з'явиться перша розмова **>150 повідомлень**. Тоді правильний порядок — **не віртуалізація першою**, а: (1) зняти `layout` + мертвий `popLayout` (дешево, ріже криву); (2) `.limit(N)` + «догортати старіші» (ріже і payload, і ноди — месенджери так і працюють); (3) віртуалізація — лише якщо перших двох не вистачило. Перевірочний запит:

```sql
with dm as (select conversation_id, count(*)::int c from direct_messages group by 1)
select coalesce(max(c),0) from dm;
```
...
🐞 **Знайдено попутно → ✅ полагоджено на прохання founder (`7a21a1a5`):** `MyLoyaltyPage.tsx` показував `Отримано: {completed × c2c_discount_pct}%` — **поточний** відсоток майстра × кількість, а не фактично нарахований `SUM(discount_pct)` (він фіксується в кожному рядку `c2c_referrals` на момент реферала). Розходилось, щойно майстер міняв свій `c2c_discount_pct` між рефералами — і суперечило балансу поруч, який рахувався правильно. Тепер сторінка віддає `earnedPct` (вона вже його рахує для балансу — просто відкидала при передачі в компонент). `c2cDiscountPct` лишено: він потрібен для тексту «Поділитись» (знижка подрузі). Верифіковано живою сторінкою: реферали 10%+15% при поточних 10% → було «Отримано: 20%», стало **25%**, Баланс 15%.

---

## Головне, що змінило картину (не повторювати помилок статичного аудиту)

Аудит писався зі статичного читання. Жива перевірка **сім разів** спростувала або звузила його: `ASSET-03` ↩️ · `DB-07` 5→2 · `RND-05` 6→3 · `ASSET-02` 9→5 · `RND-06` ↩️ · `DB-08` (міграція виявилась зайвою) · `RND-04` ч.2 ↩️. Наступна сесія має так само **перевіряти передумову перед виконанням**, а не виконувати беклог наосліп.

> **Роботі бракувало одного інструмента — заміру.** `RND-04` ч.2 закрився не читанням коду, а одним `count(*)` на проді. Де задача обґрунтована **обсягом** («список без віртуалізації», «повний скан», «N+1»), обсяг треба **виміряти на живих даних ПЕРШИМ ділом**, а не виводити з коду. Читання коду каже, що станеться при великому N; воно нічого не каже про те, чи існує велике N.

- **`OPT-RND-04` ч.2 ↩️ скасовано.** Найдовша DM-розмова на проді = **1 повідомлення**, тікет = **9**. Різати нічого. Деталі + тригер повернення (>150) — секція вище.
...
### Фаза 2 (DB/RPC) — **єдине, що лишилось. Можна закривати повністю самому**
...
> **Блокера «apply робить founder» більше нема:** прод-apply через Management API відпрацьований, реєстр міграцій чистий (`db push` = no-op). Рецепт apply + три пастки PowerShell — у `TRANSITION_PROMPT §Прод-apply`.
...
> **І перед цим — заміряй.** Кожна DB-задача обґрунтована обсягом («full-scan», «5000 рядків», «без пагінації»). Той самий `count(*)` на проді, що закрив `RND-04` ч.2, може звузити або скасувати частину з них. Прод сьогодні: 11 майстрів — тобто «5000 рядків для set-diff» майже напевно перебільшення. **Міряй перед кодом.**
