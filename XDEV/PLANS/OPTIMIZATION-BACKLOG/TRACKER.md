...
**Прогрес:** 2/18 ✅ (OPT-RND-01, OPT-RND-02)
...
| `OPT-DB-06` | `useReviews` unbounded + дубль-запит `reviews-pending` (підмножина) | P1 | ⬜ | `tanstack-query` | **Sonnet** | — |
...
| ID | Задача | P | Ст | Спеціаліст-скіли | Модель | Commit |
|----|--------|---|----|------------------|--------|--------|
| `OPT-RND-01` | `ui/Sheet.tsx` `backdrop-blur-3xl` під scale-анімацією — джанк у КОЖНІЙ модалці | P0 | ✅ | `fixing-motion-performance` | **Opus** | pending |
| `OPT-RND-02` | `context.tsx` MasterContext value міняє identity щорендер (refresh/fetchProfile без useCallback) | P0 | ✅ | `react-best-practices` `senior-frontend` | **Opus** | pending |
| `OPT-RND-03` | `ReviewsPage` `layout` на необмеженому списку під popLayout — thrash на фільтрі | P1 | ⬜ | `fixing-motion-performance` | **Sonnet** | — |
...
| ID | Задача | P | Ст | Спеціаліст-скіли | Модель | Commit |
|----|--------|---|----|------------------|--------|--------|
| `OPT-ASSET-01` | Lazy-load важких drawer'ів: BookingDetailsModal (eager у DashboardLayout), ClientDetailSheet ×4, ImageCropper | P1 | ⬜ | `react-best-practices` | **Sonnet** | — |
