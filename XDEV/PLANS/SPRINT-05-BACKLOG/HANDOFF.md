...
**🧪 АКТИВНА ГІЛКА (2026-07-08) — TEST-M6, стабілізація e2e:** Тест-фаза триває поза Sprint-05-фічами. Стан: unit **1013/1013 ✅**; **e2e повний chromium 118 passed / 8 failed / 41 skipped** (з 35→8 failed цю сесію). 🔴 **REAL-BUG SCAN: нуль регресій продукту** — усі падіння = test-rot від редизайнів (drift testids/copy, dual-tree `.first()` бере приховану копію, over-broad селектори, fixed-timeout флейк). Підозрілі перевірено НАЖИВО й очищено. **Полагоджено цю сесію (18 специфікацій, 6 комітів):** client-phone харнес-геп (розблокував усю клієнтську зону — `E2E_CLIENT_EMAIL`→timetravel-клієнт), 14-client-journey dual-tree, wizard-panel testid, auth terms-gate, 03-referral, 15/17/04 analytics/loyalty/cron, 19/20/ux-premium/zz. ✅ **ВСІ 8 ЗАКРИТО (2026-07-08):** повний chromium **123 passed / 2 failed / 42 skipped**. 10-master-bookings feature-rewrite (list/timeline/focus), 08+10 wizard step-3 (products-крок опційний), 05-reviews (5 drift-точок вкл. star-селектор «зірк»≠«5 зірок»), broadcasts (сегмент з отримувачами), 02-badge (data-testid scoping; 02:172 last_minute→**test.fixme** бо dynamic-pricing рахується server-side і не читає debug-now cookie — не баг). **Залишкові 2 failed = екологічний флейк** (05:126+broadcasts:92 проходять ізольовано; fullyParallel проти remote-прод; CI retries:2 покриває; SEC-01 усуне). **NET: 35 → ~0 код-фейлів.** ▶ **НАСТУПНЕ:** `TEST-M5` anti-drift (щоб copy/selector-дрейф падав швидко), `SEC-01` (e2e→local/dedicated DB — усуне flake+network). Сервер для e2e: `npm run build && npm run start` проти прод-БД (`E2E_ALLOW_REMOTE=true`).

> ## 🔴 TEST-M3 → знайдено й закрито ПЕРЕКРИТІ ЗАПИСИ (2026-07-12, `86ac9b2e`)
>
> Деталі, гарантії БД по кожному вектору та обидві пастки EXCLUDE — у `TRACKER.md §TEST-M3`. Тут — тільки урок, який дорого коштував.
>
> ### ⚠️ Три помилки ОДНОГО класу за одну сесію: «перевірив НЕ ТОЙ об'єкт, а висновок зробив про той»
>
...
> 3. Перевірив RLS на таблиці `bookings` і заявив P0 «публічна сторінка не бачить броней» — а візард читає **в'юху `booking_slots`** (без ПД, anon через неї броні **бачить**). Почав писати непотрібну міграцію, перш ніж це виявив.
>
> **Правило:** перш ніж оголосити баг, мертвий код або діру — **пройди ланцюг до кінця, від споживача до джерела даних**, і переконайся, що міряєш саме той об'єкт, який використовує код. Гіпотеза «таблиця X закрита RLS» не варта нічого, якщо код читає view/RPC над нею.
>
> **Спільне з `SET LOCAL`-пасткою нижче:** негативний тест, який *не впав*, ≠ захист, який *спрацював*. Треба бачити доказ спрацювання (текст відмови, код помилки), а не відсутність помилки. На проді констрейнт довів себе живим `23P01`, а не тишею.

> ## 🔓 SEC-01 (P0) — ПОВНІСТЮ ЗАКРИТО 2026-07-12
>
> Залишок, що нижче описаний як «🔄 потребує dev-машини», **виявився вже виконаним**. Звірено живими даними, а не доками:
> - коміт `41bf5b69` **у `main`** (`git merge-base --is-ancestor` → так). Нотатка «НЕ змержено» застаріла.
> - `.env.test` цілить на **`http://127.0.0.1:54321`**; `E2E_ALLOW_REMOTE` у файлі **відсутній**.
> - **Guard перевірено наживо** (не юніт-проксі): підмінив `.env.test` на прод-реф + `E2E_ALLOW_REMOTE=true` — найнебезпечніша комбінація — → `SAFETY ABORT (HARD)`, exit 1, виконання **не дійшло** навіть до «Resolving auth users». Нуль записів. Файл відновлено через `trap`.
> - **Ротація прод service-role НЕ потрібна:** ключ у `.env.test` — локальний demo (`iss=supabase-demo`); сам `.env.test` у `.gitignore` (`.env*`) і **ніколи не комітився** (`git log --all -- .env.test` порожній) → у git не витікав.
>
> ⚠️ **Пастка, на якій сам спіймався:** перша спроба негативного тесту (`E2E_ALLOW_REMOTE=true NEXT_PUBLIC_SUPABASE_URL=<прод> npx tsx …`) **нічого не довела** — `dotenvx` перезаписує `process.env` значеннями з `.env.test`, тож сідер пішов на localhost і guard навіть не запустився. Тест «відпрацював» вхолосту. Щоб перевірити guard, треба міняти **сам файл**, а не env-змінну. *(Той самий клас помилки, що `SET LOCAL` поза транзакцією.)*

**✅ TEST-M5 + SEC-01 code-guard DONE (2026-07-08, гілка `test/sec-01-guard-m5-antidrift`, коміт `41bf5b69`, ✅ згодом змержено в `main`):**
- **SEC-01 (P0) — прод-wipe дірку ЗАКРИТО в коді.** Локальний `.env.test` націлював сідер на ПРОД-реф (`sqlrxsopllgztvgrerqk`) з `E2E_ALLOW_REMOTE=true` → `npm run test:e2e` wipe+recreate `e2e_*@test.com` НА ПРОДІ. Fix: чистий модуль `src/lib/testing/e2eSeedGuard.ts` (`findProdRef`/`isProdSupabaseUrl`/`PROD_SUPABASE_REFS`) + hard-abort у `seed-e2e-data.ts assertSafeEnvironment()` **ПЕРЕД** `ALLOW_REMOTE`-байпасом (флаг більше не розблоковує прод). 5 unit-тестів зелені. **Верифіковано наживо:** `npx tsx scripts/seed-e2e-data.ts` проти реального прод-`.env.test` → abort exit 1 на рядку 92, ДО створення admin-клієнта (нуль DB-записів). tsc 0. **ЗАЛИШОК SEC-01 (🔄, потребує dev-машини):** repoint `.env.test` на локальний Supabase (`npx supabase start`, `E2E_ALLOW_REMOTE=false`) або dedicated e2e-проект; опц. ротація прод service-role ключа. Guard робить забування цього кроку БЕЗПЕЧНИМ (гучний abort замість wipe), але локальний e2e лишається заблокованим доки не repoint.
- **TEST-M5 (P2) — anti-drift.** Наземна правда: `src/middleware.ts` існує (`export middleware()` + `config.matcher`, повний role-guard); `src/proxy.ts` НЕ існує. Виправлено `proxy.ts→middleware.ts` у ЖИВИХ мапах (SYSTEM_MAP:448, AI_MASTER_GUIDE:86+265, DOMAIN_MAPS/01+14, SKILL_PROTOCOL, SKILLS_TOP200) + 5 e2e-коментарів. Історичні audit-снапшоти лишено (вже фіксують правку). Переписано `TESTING_MAP.md` під живий `find`: 47 unit-файлів (заявлено 5), 36 e2e-специфікацій (додано 11 відсутніх). 🔴 **Виправлено небезпечну брехню:** мапа заявляла глобальний wipe `profiles`/`master_profiles` — реальний сідер scoped на `e2e_*@test.com`. Усі Cyrillic-доки encoding-clean. Скоуп за AskUserQuestion: code-guard + doc-only (БЕЗ важкого TESTING_MAP CI-авто-скрипта). MemPalace: `drawer_bookit_testing_ebb4b746`.

**🎯 НАСТУПНА СЕСІЯ (за рішенням founder 2026-07-08): ЗВІРКА АУДИТУ.** Готовий чеклист → **`XDEV/AUDIT/RECON_CHECKLIST.md`** (усі P0/P1/P2 знахідки з 5 аудит-файлів + команди перевірки + передзаповнені статуси). Аудит датований 2026-06-15 (застарілий); треба пройти кожну знахідку наживо → оновити `AUDIT/*.md` → видати чистий список «що реально ще відкрито». VERIFY-прохід, БЕЗ коду. Передзвірено цю сесію: 2 P0-security ✅FIXED (fire-notifs 404+DEBUG_TOKEN, telegram webhook secret), UX A2/A3/A4/A8/A10 ✅DONE(Sprint-05). Ймовірно OPEN: P0-TEST-1/2/3 (E2E /my/messages + /explore + vitest coverage config), P0-PERF-1 (explore force-dynamic кеш), webkit/mobile flake, частина admin a11y (ARIA/type=button/hardcoded tokens). Також FAB-фікс 10-master-bookings ✅ (`a1cd2571`, детермінований test-баг не флейк) + продукт-знахідка: auto-day-focus не спрацьовує в manual-master візарді (окрема задача, не блокер).

**Наступна задача (Sprint-05 фічі):** Sprint-05 задачі закрито. Залишкові борги (НЕ нові задачі, а операційні кроки): (1) 🔴 застосувати NP-міграцію; (2) деплой батчем `vercel --prod`; (3) опц. ТТН-фаза M-SHOP-05. **C-DESK-01 ПОВНІСТЮ ЗАКРИТО** (усі 8 стор). **M-SHOP-05 MVP ЗАКРИТО** (`1cf2746d`).

**✅ M-SHOP-05 MVP DONE (commit `1cf2746d`, НЕ задеплоєно, міграція НЕ застосована):** Nova Poshta пошук міста+відділення в shop-checkout. **Обсяг обрано founder (AskUserQuestion):** MVP пошук міста/відділення; ТТН/трек відкладено (потребує sender-профілю майстра — контрагент NP, адреса відправлення, хто платник — цих даних у моделі майстра НЕМАЄ). **Зроблено:** `lib/novaposhta/client.ts` (server-only NP API-клієнт: `getCities`+`getWarehouses`, read-only, ключ `NOVAPOSHTA_API_KEY` у `.env.local` git-ignored, `next.revalidate:3600`) · `lib/actions/novaPoshta.ts` (server actions `searchNpCities`/`getNpWarehouses`) · `shop/NovaPoshtaPicker.tsx` (місто-autocomplete debounce 300 → вибір відділення/поштомату; outside-click close) · `createOrder` валідація `np_warehouse_ref` + insert 4 np-полів; `delivery_address` = readable «Місто, Відділення» (майстер бачить у OrderCard без змін) · `ShopCartBar` textarea→picker · Order type +np-поля · міграція `20260706000000` (additive nullable np_city_ref/name, np_warehouse_ref/name). **✅ Міграція застосована (2026-07-06):** через Supabase Management API (POST `/v1/projects/sqlrxsopllgztvgrerqk/database/query` з Bearer PAT) — 4 колонки `np_*` верифіковано на прод-БД. CLI залінковано, `SUPABASE_ACCESS_TOKEN` у `.env.local` (git-ignored). **🔴 Борги:** (1) додати `NOVAPOSHTA_API_KEY` у Vercel env (для проду); (2) ТТН/трек = наступна фаза. **Гейти:** own-eyes з ЖИВИМ NP API (Львів→реальні міста з областями→відділення/поштомати→payload з реальними Ref `81ce4e7c`/`4796602e`+назви) · TSC:0 · Build:clean · encoding чистий. **🔧 Уроки:** (1) ключ NP валідний — перевірено живим `getSettlementAreas`; "Data is invalid" у curl був артефактом Windows git-bash Cyrillic-кодування, у Node fetch UTF-8 коректний; (2) `.env.local` append НЕ переживає reset сесії — перевіряти `grep -c NOVAPOSHTA_API_KEY .env.local` перед own-eyes, dev читає env лише на старті; (3) ТТН NP вимагає sender counterparty/CitySender/SendersPhone — не будувати наосліп.

**✅ C-DESK-01 стор.5-8 DONE цю сесію (НЕ задеплоєно):**
- **стор.5 /my/notifications (`304141b6`):** читацька поверхня — desktop `hidden lg:block` max-w-4xl + 2-кол `grid-cols-[220px_1fr]`: ліва sticky summary-рейка «Огляд» (Нових/Потребують відповіді/Усього, калібр-тон #9A4508) + права вузька колонка (consent + дата-групи фіду). Винесено ConsentCard/NotifRow/FeedGroups/EmptyNotifs для переюзу. Мобільний `lg:hidden` недоторканий.
- **стор.6 /my/profile (`d9f8d555`):** 2-кол на тому ж дереві (форма-стейт спільний) — identity editorial-cover `lg:sticky` ліворуч + форма праворуч (обгорнуто всі секції в праву колонку, мобільний DOM-порядок збережено). save-FAB `lg:left-[264px]` не під sidebar + max-w-md.
- **стор.7-8 /explore + /shop (`c3481f38`):** widening — контейнери max-w-2xl/lg → `lg:max-w-5xl`, гріди карток/товарів `+lg:grid-cols-4`. Shop cart-піл: globals `body.has-client-shell .shop-cart-bar{left:264px}` (authed only, guest недоторканий), checkout-логіка ShopCartBar не чіпана.
- **Гейти всіх:** own-eyes Playwright desktop 1280/1440 + mobile 390 (недоторканий) · encoding чистий · TSC:0 · Build:clean.
- **Уроки:** (1) desktop-стратегія за складністю — окреме `hidden lg:block` дерево лише при кардинально іншій структурі, прості = responsive-класи; (2) shop cart-піл / profile FAB на десктопі не мають лізти під sidebar → `lg:left-[264px]` (статик для profile-FAB бо тільки authed /my) або `body.has-client-shell` CSS (shop бо guest+client); (3) винесення під-компонентів (notifications) = чистий mobile+desktop переюз без дублювання markup.

**✅ C-DESK-01 Фаза 2 стор.4 /my/loyalty DONE (commit `f5795c15`, НЕ задеплоєно):** десктоп-адаптив + **повний emoji-purge (директива founder «прибирай емодзі всюди»)**. **No-Emoji:** усі avatar-заглушки з emoji (`masterEmoji` fallback) → монограм-обкладинка через новий спільний `MasterAvatar` (Frost hue `linear-gradient` + Cormorant ініціал, reuse `monogramHue`/`initialOf`/`CORMORANT` з `explore/shared`): loyalty 3 місця (програми/C2C-реферали/промокоди) + masters мобільна картка (`{master.avatarEmoji}` → `MediaCover`). **Десктоп-адаптив loyalty** (структура тримається — БЕЗ окремого десктоп-дерева): root `lg:max-w-5xl lg:mx-auto` центр + `lg:px-8 lg:py-8`, таби/sub-таби `lg:max-w-md`, гріди програм і C2C-майстрів `grid sm:grid-cols-2`, C2B-колонка `lg:max-w-2xl`. `/my/loyalty` додано в `isWideDesktopRoute`. **Гейти:** own-eyes Playwright (loyalty/referral-C2C/C2B @1280 + mobile 390 недоторканий, монограми скрізь) · a11y = established токени · encoding чистий (2 наявні UI em-dash у pre-existing копії — поза скоупом) · TSC:0 · Build:clean. **Уроки:** (1) 🔴 ЖОДНИХ emoji-fallback для аватара — завжди designed монограм (спільний MasterAvatar, reuse explore/shared); (2) десктоп-адаптив простих сторінок = центрований max-w + responsive гріди, окреме `hidden lg:block` дерево тільки коли структура кардинально інша (bookings/messages/masters); (3) `masterEmoji` поле лишилось у типах/запиті (дані), просто не рендериться — чистити тип окремо не варто (revenue-neutral).

**✅ C-DESK-01 Фаза 2 стор.3 /my/masters DONE (commit `ac940ef6`, НЕ задеплоєно):** десктоп editorial-галерея (lg+). Мобільний 2-col грід `lg:hidden` недоторканий (включно emoji-fallback) + окреме `hidden lg:block` десктоп-дерево (патерн DesktopBookings). **Композиція:** темний `ClientPageHero` band (спільний) + **featured «Твій головний майстер»** (найчастіше відвіданий, tiebreak recency): горизонтальна світла картка — велика монограм/фото-обкладинка + серіф-ім'я 3xl + метрики (`metric-value` візити · востаннє · місто) + категорії-піли + Записатись/Написати; далі секція **«Інші майстри»** = auto-fit грід `minmax(258px,1fr)` компактних карток. **No-photo → монограм-обкладинка** (детермінований Frost hue `linear-gradient` + Cormorant ініціал), reuse `monogramHue`/`initialOf`/`CORMORANT` з `components/public/explore/shared.ts` — НЕ emoji-заглушка (No-Emoji + патерн Explore). `/my/masters` додано в `isWideDesktopRoute` (my/layout.tsx) для повної ширини на lg. **Гейти:** own-eyes Playwright (1280/1440 featured+галерея + mobile 390 недоторканий) · humanizer (3 нові рядки clean; наявний em-dash у копії empty-state→кома) · a11y = established токени · encoding чистий · TSC:0 · Build:clean. **Уроки:** (1) новий wide-десктоп-роут ТРЕБА додати в `isWideDesktopRoute`, інакше затиснутий у max-w-lg; (2) no-photo б'юті = designed монограм (reuse explore/shared), не emoji і не «сумна заглушка»; (3) featured-селекція = найсильніший сигнал relationship (most-visits), не recency; (4) мобільний недоторканий = навіть emoji-fallback лишаємо (десктоп монограм — свідома breakpoint-різниця).

**✅ C-DESK-01 Фаза 2 стор.2 /my/messages DONE (commit `9386a9d7`, НЕ задеплоєно):** десктоп 2-pane месенджер (lg+). **Рішення founder (AskUserQuestion):** client-side pane-swap (без route-переходу, `?c=` deep-link) · editorial empty-state · **support теж у праву панель**. **Ключове відкриття скоупу:** `MessagesListPage`/`DirectChatPage`/`SupportChatPage`/`ChatShell` — SHARED між клієнтом (`/my/*`) і майстром (`/dashboard/*`); C-DESK-01 = тільки клієнт → 2-pane зроблено окремим `MyMessagesDesktop.tsx` (`hidden lg:block`, патерн DesktopBookings), shared-примітиви НЕ переписані. **ChatShell `contained`-режим:** fill-pane `h-full` замість fixed-fullscreen + `useChatViewport(enabled=false)` (інакше хук лочить `document.body.overflow=hidden` глобально на десктопі). `DirectChatPage`/`SupportChatPage` отримали `inPane`-проп (contained + без back). Default off усюди → мобільний/майстер byte-identical. **Права панель:** selection client-state (support | dm | null→EmptyPane), keyed motion.div remount на switch (fresh realtime-subscription), unread обнуляється оптимістично при відкритті. «Написати майстру» = `getOrCreateConversation`→select-in-pane→`router.refresh()`. **🔴 Latent-баг знайдено й полагоджено:** офсет-правило C-DESK-01 додає лише `pl-264px` на root `<main>`, але `my/layout.tsx` затискав контент у `max-w-lg` (512px) — тобто десктоп-bookings master-detail у РЕАЛЬНОМУ layout стиснутий (own-eyes робили в ізольованому прев'ю→не спливло, ще не задеплоєне). Фікс: layout-контейнер route-aware — `/my/bookings`+`/my/messages` повна ширина на lg; messages додатково = full-height focus (прибрано phantom `md:pt-20` gap + плаваючий SupportWidget/ChannelBanner на lg через `lg:hidden`-обгортку, мобільний недоторканий). **Гейти:** own-eyes Playwright (empty/DM/support @1280·1440 + mobile 390 = 2-pane схований) · humanizer (2 нові рядки, em-dash прибрано, ти/ви узгоджено з наявною messages-поверхнею = ВИ) · a11y = established токени · encoding чистий · TSC:0 · Build:clean. **Уроки:** (1) перед редизайном chat/shared-поверхні — grep, чи не юзає майстер; якщо так → opt-in проп default-off, не переписувати; (2) `useChatViewport` має side-effect (body-scroll lock) → contained-режим мусить його вимикати; (3) sidebar-офсет `pl-264px` НЕ достатній сам по собі — `max-w-lg` внутрішній контейнер треба widen route-aware, інакше десктоп master-detail затиснутий; (4) deep-link chat на десктопі = `?c=` param (client-state), не окремий route — уникає fullscreen-стрибка + SSR-churn.
...
- **Форма (кожна секція свій світ, copy 1:1):** Features→темний флагман-блок (Смарт-слоти)+hairline-список 5 фіч (замість 6 icon-card клонів+пастель-хардкод); Agitation→hairline-список болів, 1-й featured (без 01/02/03); ClientFlow→горизонтальний connected-степер, темні вузли+detail-чіпи; Process→вертикальний spine-таймлайн, обведені indigo-вузли; Magic→редакційні pull-stat рядки (indigo серіф-цифра+серіф-заголовок, без hero-metric боксів); Bento→editorial stat-strip featured «до 27%»+тижнева сітка-герой.
- 🔴 **card-rise фікс:** `LandingClientFlow` → `overlap:false` у `LandingPageContent SECTIONS` (горизонтальний степер короткий по вертикалі → card-rise наступної секції [ефективно ~22vh+тінь] перекривав текст-під-вузлами; Process працює саме бо `overlap:false`). Це також зняло rise у Comparison — ОК, тече чисто.
- **Eyebrow-граматика:** знято uppercase-tracked kicker з УСІХ 13 секцій; лишився ЛИШЕ Hero-бейдж «для майстрів краси» як свідомий бренд-голос (функціональні лейбли — TrustBar-стати, тарифні STARTER/PRO, колонки Comparison, Зараз/З Bookit — НЕ чіпані).
- **Economy:** eyebrow + «мертва зона» (насправді WordLine-клипінг+гейт, зникла після П0) + mobile result-картка clamp 2.4→1.75rem (два числа+×1.27 не влазили в 390px).
- **Асет:** `public/landing/dashboard.png` — вшиту браузерну плашку «F11» замальовано кольором навбар-пілюлі `#DDE4FE` через `sharp` (Next image-кеш `.next/cache/images` треба чистити щоб побачити). Плоский патч поверх frost-градієнта, на розмірі мокапа непомітно; піксель-перфект → founder дасть чистий скрін.
- **G-LAND-01 `LandingForWhom.tsx`:** редакційний типографічний індекс спеціалізацій (не icon-картки) — core-ніші крупний Cormorant (Манікюр/Брови indigo-акцент), суміжні дрібніші muted, розділові крапки, closing-рядок трейлить. Після Marquee, поза card-rise. Copy через humanizer (em dash прибрано). Скіли: impeccable(critique/audit/polish) + humanizer + landing-page-guide-v2.
- Own-eyes: КОЖНА секція рендерена локально Playwright mobile 390 + desktop 1440, founder-закон. TSC:0 Build:clean. Деплой чекає команди. **Уроки:** (1) card-rise `overlap:true` з'їдає низ секції ~22vh+тінь → короткий контент виводити `overlap:false`; (2) per-word `overflow:hidden` mask ріже кириличні виносні при tight leading — фатально для UA; (3) `reveal` мусить підсилювати вже-видиме (`initial={false}`/keyframe), не гейтити на inView; (4) Next `<Image>` кешує оптимізований варіант — заміна файлу без чистки `.next/cache/images` не видно.
...
**✅ C-EXPL-01 (+C-EXPL-02) DONE (commit `373e9701`, НЕ задеплоєно):** /explore «Пошуковий портал» — повний концепт з нуля (Закон темного блоку), тип REDESIGN Tier 2. **Пре-код ритуал повний:** brainstorming (пошук-герой, напрям D1+D3) → impeccable (craft, delight) → grilling → бриф → APPROVE. **Концепт:** темний slate-портал (`var(--accent)` #0F172A, рідний Frost) поглинає весь верхній chrome-стос; герой = великий пошук; Great Vibes-заголовок «Знайди свого майстра» (founder: прописний як було); категорії-типографіка ОДНАКОВОГО розміру з superscript-лічильниками (founder відкинув розмірні тіри). **Sticky-компакт пошук** при скролі (IntersectionObserver на sentinel, `md:hidden`, під safe-area). **Інтент-грід (D3):** герой-плитка з реальних даних «Працюють сьогодні · N» (actionable, не hero-metric-brag; чесний fallback today→tomorrow→top при 0) + чіпи Поруч/Завтра/PRO/Топ-рейтинг; ціна/сорт/«з відгуками» → **vaul BottomSheet «Фільтри»** (реюз `ui/Sheet`, лічильник активних). **Spotlight:** 1 топ-PRO з реальним фото (за ratingCount), дедуп з гріду, зникає якщо жоден PRO не має фото. **Картки з нуля (C-EXPL-02 злито):** MasterCard/MasterListCard/SpotlightCard, однаковий frame; 🔴 no-photo → **монограм-обкладинка** (детермінований hue в Frost indigo-violet band, serif-ініціал) замість «сумної E»; спеціалізація → **темний slate-пілл** (founder); подвійний PRO прибрано. 🔴 **Чесність даних:** `availableToday`/`Tomorrow` = «працює в цей DOW» (`schedule_templates.is_working`), НЕ вільний слот → лейбли «Працює сьогодні/завтра» (не «Є слот»); реальні слоти = майбутній DATA-таск. **Токени:** усі хардкод indigo/emerald/amber/black → Frost CSS-vars. **Рефактор:** 1290-рядковий `ExplorePage.tsx` → контейнер стану + `explore/{shared,cards,SearchPortal,IntentGrid,FilterSheet}`. **PublicNavbar** не чіпано — вже токен-чистий (slate-CTA); вигадувати зміну = порушення. Десктоп mobile-first (повний desktop → C-DESK-01). Скіли: brainstorming→impeccable(craft,delight)→grilling→design-taste-frontend+emilkowalski-motion→humanizer. Локальний рендер власними очима (Playwright headless iPhone13 @ мок `/explore-preview` поза auth, `data-theme=frost` wrapper форсує slate; видалено перед комітом) + founder на реальному девайсі (16 майстрів, дав 3 правки — усі застосовані). TSC:0 Build:clean. Бриф: BRIEFS/C-EXPL-01.md. **Уроки:** (1) `availableToday` = працює-в-DOW, НЕ вільний слот — чесний лейбл замість вигаданого; (2) no-photo в б'юті = designed монограм-обкладинка (data-hue), не сумна заглушка, і НЕ ламає порівнянність гріду; (3) рівномірна сітка результатів = ЛЕГІТИМНА (порівняльна поверхня) — асиметрія на рівні СТОРІНКИ (портал/інтент/spotlight/грід), не ціною зручності порівняння; (4) прев'ю-роут поза (master)/(client) — сам `data-theme=frost` wrapper форсує тему без auth; (5) видалив прев'ю-роут → build падав на стале `.next/dev/types` → `rm -rf .next` + ребілд.
