...
- **Навбар-стрибок** (`cf801b34`, MobileHub.tsx): fixed-навбар разово «зависав» при скролі → прибрано framer `layout` з motion.div смуги (layout-projection міряла проти скрол-офсету). ⚠️ iOS-транзієнт, підтвердити на девайсі. Реальний навбар = `MobileHub`→`NavBar` (не BentoBottomNav).
- **Локація редизайн + карта** (`6daa465b`, LocationWidget.tsx + LocationPicker.tsx): карта падала (Google білінг/ключ = інфра founder). Рішення founder: лишити Google + повний редизайн. Hero-адреса + форма + карта-герой + deep-link «Відкрити в картах» (працює завжди). Спільний premium MapFallback (замість сирого Google-боксу) для no-key/error + skeleton-loading + sync пошук→форма + a11y (#A8928D/muted→text-text-sub). Карта оживе коли founder полагодить білінг у Google Cloud.
- **Date/time інпути overflow** (`8694b2fe`, follow-up M-SET-04, globals.css + VacationManagerView.tsx): нативні iOS date/time мають фіксовану інтринсік-ширину → вилазили за блок + накладались. Fix: `input[type=date|time]{min-width:0;max-width:100%}` глобально (покриває графік/перерви/відпустки) + min-w-0 комірки/інпути + пара дат стек на мобілці (grid-cols-1 sm:grid-cols-2) + компактні time-інпути (w-30, «не треба багато місця для часу» — founder). ⚠️ iOS-специфіка, підтвердити на девайсі.
