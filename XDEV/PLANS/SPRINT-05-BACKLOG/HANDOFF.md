...
**🧪 АКТИВНА ГІЛКА (2026-07-08) — TEST-M6, стабілізація e2e:** Тест-фаза триває поза Sprint-05-фічами. Стан: unit **1013/1013 ✅**; realtime-краш логіну + 3 латентні міни пофікшено; RLS 8/8 verified; **e2e 8/35 rotted-специфікацій полагоджено** (smoke+booking-flow). ▶ **НАСТУПНЕ в новій сесії:** продовжити TEST-M6 за `BRIEFS/TEST-M6.md`, починаючи з **client-phone харнес-гепу** (сідер не створює `e2e_client@test.com` з телефоном → `client.json` не будується → всі клієнтські специфікації скіпляться). Повний план: `XDEV/PLANS/TEST_COVERAGE_MATRIX.md`. Задачі: `TEST-M3/M4/M5`, `SEC-01` (env→прод, deferred). MemPalace: перевести на venv-python (SAC блокує shim-exe) — див. `project_mempalace_setup_fix.md`.
