...
**Наступна задача:** **`C-NAV-01`** — MyBottomNav FAB redesign (справжня клієнт-нижня-навігація; перенести інтерим-fab «Чат» з M-HELP-02b у bottom-nav; P1, клієнт-зона B).

**🔧 Ad-hoc фікс (репорт founder зі скріна, commit `87b16079`, НЕ задеплоєно):** iOS caret плавав над інпутом на УСІХ чат-інпутах (тап → caret вище, у зоні повідомлень; друк → стрибає в інпут). **Root cause:** `overflow:hidden` на body НЕ блокує iOS-скрол layout-viewport при фокусі інпута всередині `position:fixed` ChatShell — iOS скролив body, малював caret у зскроленій системі координат, а `window.scrollTo(0,0)` знімав скрол → caret лишався на старому місці (перемальовується лише при зміні тексту). **Fix:** `useChatViewport` локає сторінку **fixed body** (`position:fixed` + `top:-scrollY` + overflow hidden на html/body), прибрано `scrollTo(0,0)`; скролити нема що → caret тримається з інпутом, гепа теж нема; scrollY відновлюється на unmount. TSC:0 Build:clean. ⚠️ Механізм-фікс, device-QA за founder (headless caret не відтворює). **Урок:** для повноекранного `position:fixed`-інпута на iOS `overflow:hidden` замало — тільки `position:fixed` body реально блокує focus-scroll (і геп, і caret-десинк).
...
🔴 **NEXT `C-NAV-01`:** MyBottomNav клієнта (Записи | Бонуси | FAB | Чат | Профіль) — тут перенести інтерим плаваючий `InboxNavButton` (M-HELP-02b) у справжню bottom-nav.
