...
**Прогрес:** 66/86 ✅ · 3 ↩️ скасовано (`M-DASH-11` + `M-MKT-01`/`M-MKT-02` поглинуто редизайном M-MKT-04, founder) — **Фаза 3: Analytics 7/7 ✅. Фаза 4: M-SET-01..05 ✅ · M-BILL-01 ✅ (задеплоєно) · M-BILL-02 ✅ (НЕ задеплоєно) · M-PORT-01 ✅ · M-PORT-02 ✅ · M-ORD-01 ✅ · M-DOC-01 ✅ (НЕ задеплоєно).** Total 84→86 (+M-SHOP-05 Нова Пошта, +M-CLI-07 CSV — чесні борги з M-BILL-02).
**Наступна задача:** **`M-HELP-01`** — Підтримка/Академія: дедуп + навігація (P1, Sonnet). По порядку трекера у Фазі 4. Ціль: сторінка підтримки/академії майстра (знайти компонент — імовірно /dashboard/help або /dashboard/support/academy). «Дедуп» = ймовірно дубльований контент між Підтримкою та Академією. Task Gate: живий код (де обидві сторінки, що дублюється) → тип (REDESIGN + можливо копі) → бриф → OK founder → код.
