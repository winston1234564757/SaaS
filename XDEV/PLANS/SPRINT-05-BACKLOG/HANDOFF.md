...
**Прогрес:** 51/84 ✅ · 3 ↩️ скасовано (`M-DASH-11` + `M-MKT-01`/`M-MKT-02` поглинуто редизайном M-MKT-04, founder) — **Фаза 3: Revenue 6/17 done; Growth 2/2 done; Marketing 4/6 done + 2 ↩️; Reviews 2/2 done; Analytics 2/7 done (Огляд + Джерела).**
**Наступна задача:** **`M-ANL-03` — Аналітика: таб «Відгуки» (`reviews`) editorial-redesign** (`design-taste-frontend` + `impeccable` · Opus · P1). Бриф: `ANALYTICS_TABS_REDESIGN.md` §3 + §0. **Темний герой — alternating** (Джерела світлий → Відгуки темний). Аналітика = 7 табів-задач (один таб = одна сесія). **Секції Marketing + Reviews закриті.**
**Оновлено:** 2026-06-30
...
## ▶ NEXT: `M-ANL-03` — Аналітика: таб «Відгуки» (`reviews`) редизайн (P1)

**Тип:** REDESIGN · **Тір:** 2 · **Скіли:** `design-taste-frontend` + `impeccable` · **Модель:** Opus · **Бриф:** `ANALYTICS_TABS_REDESIGN.md` §3 («Відгуки») + §0 (Спільна основа — читати ПЕРШИМ).

**🎨 Alternating темний герой:** Огляд темний, Джерела світлий → **Відгуки = ТЕМНИЙ герой** (рішення founder проти втоми від темного, темний через таб). Темний hero-блок = дзеркало `OverviewBriefing` (`--hero-card-bg` #0F172A + `--accent-on` cream; білі alpha: `/55`=6.09 ✓, `/45`=4.46 ✗).

**Закон редизайну (Принцип темного блоку):** проектуй з концепту з нуля (НЕ ретрофіт), асиметрична ієрархія герой+решток у КОЖНІЙ секції, маркер провалу = рівномірність, **рендер локально власними очима ДО показу founder**.

**Воркфлоу верифікації (відпрацьовано на M-ANL-02):** прев'ю-роут НЕ під `_`-папкою (Next App Router виключає `_folder` з роутингу → 404). Клади в `src/app/devpreview/<tab>/page.tsx`, розділяй компонент на `<Tab>` (fetch) + `<Tab>View` (презентація) → прев'ю рендерить View з мок-даними. Скрипт-скріншот: тимчасовий `.mjs` у `bookit/` (ESM не бачить node_modules ззовні), `npx playwright`. Видаляй прев'ю-роут + скрипти ПЕРЕД commit.

**Переюз:** `SectionHeading`, `OverviewDetailSheet`, патерн `Featured*`, `OverviewBriefing` (зразок темного героя), `BentoCell`.

**Дизайн-мова:** Frost-токени; числа `metric-value` tabular; заголовки `heading-serif`; текст `text-foreground`/`text-text-sub` (#475569), **заборонено** `text-muted-foreground/60` на світлому (2.76 провал); a11y графіка 3:1 / текст 4.5:1 через `mcp__a11y`. **🔴 Δ-зелений: `#0D6B2F` (5.21), НЕ `text-success` #16803C (3.93 провал малого тексту на periwinkle).**

**Порядок решти табів (один = одна сесія):** ~~Джерела~~ ✅ → Відгуки (03) → Склад (04) → Фінанси (05) → Поведінка (06) → Зростання (07). Чергування героя: темний(Огляд)·світлий(Джерела)·темний(Відгуки)·світлий(Склад)·темний(Фінанси)·світлий(Поведінка)·темний(Зростання).

**🔴 Фейки до виправлення:** FinancesTab/StockTab dummy-числа під блюром (M-ANL-04/05 → skeleton без чисел); VacationTab фейк-кнопка «Оптимізувати розклад» + вигадка `lost*0.4` (M-ANL-06 → чесний аналіз або реальна дія).
