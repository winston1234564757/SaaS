...
**Прогрес:** 11/74 ✅ (`G-LAND-02` · `M-SVC-01` · `M-DASH-06` · `M-SHOP-04` · `G-LOGIN-02` · `G-PWA-02` · `G-PWA-01` · `M-DASH-01` · `M-DASH-02` · `M-DASH-03` · `M-DASH-04`)
**Наступна задача:** **`M-DASH-05` — Дашборд: "Доходи і записи" — колоризація + fix "грн"** (`impeccable colorize` · Sonnet · P1)
**Оновлено:** 2026-06-24
...
## ▶ NEXT: `M-DASH-05` — Дашборд: "Доходи і записи" — колоризація + fix "грн"

**Тип:** REDESIGN/colorize (Tier 1-2) · **Скіл:** `impeccable (colorize)` · **Модель:** Sonnet · **P1** · **Фаза 2**

**Задача (з BACKLOG):** блок «Доходи і записи» (тижневий чарт із перемикачем «Записи / Дохід», `widgets/frost/WeeklyChartWidget.tsx`):
1. Колоризувати стовпці статус-кольорами — **ті самі, що на сторінці Клієнти, блок «Здоров'я бази»** (знайти джерело палітри там, переюзати, не вигадувати нові).
2. Fix переносу «грн» у сумі доходу (грн відривається на новий рядок / ламає верстку — ймовірно `white-space`/`flex-wrap` на числі+валюті).

**Підхід:**
1. Read `widgets/frost/WeeklyChartWidget.tsx` + знайти палітру статус-кольорів у Клієнти/«Здоров'я бази» (grep `RETENTION_CONFIG`/health у `master/clients/`).
2. Skill `impeccable` subtool `colorize` — узгодити мапінг кольорів зі статусами.
3. «грн» fix — `whitespace-nowrap` на обгортці суми або `formatCurrency`/`formatPrice` (вже є в `utils/currency.ts`).
4. tsc + (Tier 1-2 → повний build обов'язковий).
