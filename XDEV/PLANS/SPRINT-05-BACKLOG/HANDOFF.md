...
## ✅ DONE: `M-DASH-12` — Дашборд десктоп: висота + кольори (ad-hoc) · commits `649d9341` (висота) · `fb2a1d71`→`f477c7e9` (бари)

**Виконано і лишилось:**
- **Вирівнювання висоти блоків** (`FrostDashboard.tsx` `FrostDesktop`): прибрано `items-start`, на grid-item div додано `h-full [&>*]:h-full` у всіх 2-3-колонкових рядках → праві блоки тягнуться на висоту лівих сусідів. Календар не форсили (задає висоту, 5↔6 тижнів). **Лишилось на проді.**
- **Бари WeeklyChart** → суцільні кольори (`background: heat`, без `color-mix`-прозорості). **Лишились на проді — відкрите питання founder.**
