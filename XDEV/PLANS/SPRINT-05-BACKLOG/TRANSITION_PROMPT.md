...
═══ ПОТОЧНИЙ СТАН ═══
Прогрес: 34/78 ✅ · 1 ↩️ (M-DASH-11 скасовано) | Sprint-05 IN PROGRESS
Наступна: M-SHOP-03b — Магазин: система відгуків про товари (create-migration + security-review + senior-backend · Opus · P1). Спека готова в BRIEFS/M-SHOP-03.md Фаза C: нова таблиця product_reviews (FK order_items, UNIQUE 1 відгук/позицію), RLS, RPC get_product_reviews (дзеркало get_service_reviews), action submitProductReview (валідує: завершене замовлення клієнта, без дублю), prompt у MyBookingsPage таб Магазин (завершені замовлення → лишити відгук на кожен товар), показ у ProductDetailView (зараз статичний порожній стан).
