...
| `M-DASH-10` | P1 | Блок "Записи" (TodaySchedule): header → uppercase (стандарт віджетів Frost, збіг із «Вільно сьогодні»); порожній стан → багатий, висота=«Вільно сьогодні», текстові заглушки + рекомендації/CTA по вкладках. ⓘ Ad-hoc від founder (2026-06-25). | `impeccable (colorize)` + `humanizer` |
| `M-DASH-11` | P1 | Блок "Пікові години" (heatmap): колоризувати клітинки heat-палітрою як у «Доходи і записи» (RETENTION_CONFIG). Порожні години = нейтрал (не червоні), колір лише де є записи: рідко→помаранч, середньо→бірюза, пік→зелений. ⓘ Ad-hoc від founder (2026-06-25). | `impeccable (colorize)` |
...
| `M-CLI-05` ✅ | P1 | Кольорова корекція карток клієнтів: прибрати фіолетове тіло + нагромадження. DONE via С-CLI-01 Етап B (`72c2b214`): де-фіолетовано тіло (periwinkle→rgba(255,255,255,.72)+status-радіал statusGlow), VIP-піл softened, числа metric-value. | `impeccable (distill + colorize)` |
| `M-CLI-06` ✅ | P1 🔄 | Сторінка клієнта (деталі) — глибокий редизайн. DONE `1f05146a`: профіль-картка (рішення founder), спільні `ClientIdentityHeader`/`ClientStatChips` (покрили 6 точок виклику + BookingDetailsModal), реальний LTV (total_spent+ранг+каденс), реальні мітки (міграція `vibe_tags text[]`). | `design-taste-frontend` + `impeccable` |
| `M-CLI-07` | P2 | CSV-експорт клієнтської бази (реальна реалізація + Pro-гейт). `UpgradePromptModal` заявляє його як Pro-фічу, але коду enforcement НЕ знайдено — або реалізувати по-справжньому, або прибрати з обіцянок. До реалізації — НЕ заявляти в маркетингу тарифів. | `senior-frontend` + `senior-backend` |
...
| `M-SHOP-05` | P2 | Доставка Новою Поштою (реальна інтеграція, зараз НЕ реалізовано): NP API (пошук міст + відділень/поштоматів), розрахунок вартості доставки, створення ТТН, трек-номер клієнту. Поле `ships_nova_poshta` уже є в профілі, але API-шару немає. Гейт: Pro. Джерело: НЕ заявляти доставку в маркетингу до реалізації. | `senior-backend` + `payment-gateway-integration` |
...
- **Усього:** 74 базові задачі (+3 ad-hoc M-DASH-10/11/12 = **77** у TRACKER) · з них **5 P0-багів** · 🔄 = повний редизайн/нова фіча (старт з `brainstorming`) · `❓` = потрібен QA Gate перед брифом (14 задач)

> ✅ `TRACKER.md`, `HANDOFF.md`, `TASK.md`, `TRANSITION_PROMPT.md` синхронізовані під систему ID (M-/C-/G-) і фазовий порядок. Живий статус і ▶ NEXT — у `TRACKER.md` (наразі **25/77 ✅** · 1 ↩️, NEXT = `M-BOOK-02`).
