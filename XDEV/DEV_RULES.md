...
- Backdrop-filter — уникати на мобільних де це спричиняє frame drops
 ...
 ## 16. Стандарт Side-Stripe (Акцент статусу)
 
 Всі картки з кольоровим індикатором статусу (BookingCard, Stats cards) повинні використовувати `border-left` замість внутрішніх `div`.
 
 ```tsx
 // ✅ ПРАВИЛЬНО
 <div className="bento-card" style={{ borderLeft: `3px solid ${color}` }}>
 ...
 **Причина**: Коректна обробка закруглених кутів (`border-radius`) без "прорізів".
