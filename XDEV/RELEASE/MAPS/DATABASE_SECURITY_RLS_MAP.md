...
2.  **`check_and_log_sms_attempt(p_phone text, p_ip text)`**
    *   *Дія*: Перевіряє та фіксує ліміти відправки SMS OTP.
    *   *Безпека*: Використовує Advisory Lock для запобігання Race Condition атакам на безкоштовне виснаження SMS балансу.
