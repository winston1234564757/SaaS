...
> Оновлено: 2026-06-30 · Джерело: живий код (v9.0.0) · Останній код-коміт: `987b3d67` (M-ANL-07: таб «Зростання» editorial-redesign — концепт «двигуни росту»; **Analytics ПОВНІСТЮ закрита 7/7**; GrowthTab→ТЕМНИЙ герой [додатковий дохід розумних цін +₴N домінанта + by-numbers, fallback uplift=0→LTV+нудж] + LTV Парето featured [концентрація%+гістограма static+winback клік→профіль] + Канали залучення [флеш/розсилки/реферали/постійні клієнти диференційовані рядки] + Спрацювання цін + Що бронюють разом смугами + Когорти reuse CohortHeatmap; 🔴 чесні лейбли «Постійні клієнти/N візитів» замість «балів» [хук рахує total_visits]; 🔴 static bars замість framer animate-width LtvHistogram/ServicePairingMatrix; консолідація GrowthTab/GrowthTabView, ВИДАЛЕНО 8 мертвих файлів [BentoSecondary, GrowthLists, DynamicPricingUplift, FlashDealsCard, BroadcastEngagement, LtvConcentration, ServicePairing, LtvHistogram]) · попередній: `f21d84fd` (M-ANL-06: таб «Поведінка» editorial-redesign — концепт «ритм бізнесу»; BehaviorTab→СВІТЛИЙ герой [теплова карта завантаження col-7 + пік-інсайт домінанта найзавантаженіший день+occupancy%] + No-Show герой секції [рейти+журнал інцидентів клік→профіль клієнта] + Lead time врізка [розподіл смугами СТАТИЧНОЇ ширини] + Vacation тиха врізка [чесний недоотриманий дохід + лінк /settings#vacations]; 🔴 ФІКС ФЕЙКУ: прибрано VacationTab «Рятувальник» цілком [кнопка-тост + lost*0.4 вигадка]; консолідовано 3 під-таби NoShowTab/LeadTimeTab/VacationTab+інлайн heatmap у BehaviorTab/BehaviorTabView, старі 3 файли ВИДАЛЕНО; Pro-gate value-екран) · попередній: `bbd45815` (M-ANL-05: таб «Фінанси» editorial-redesign — концепт «P&L звіт»; FinancesTab→ТЕМНИЙ герой [чистий прибуток domінанта metric-value + Δ до минулого періоду Pro через 2-й виклик useAnalyticsExtras з previousWindow + by-numbers] + чесний КАСКАД руху грошей [світла секція, hairline-смуги СТАТИЧНОЇ ширини НЕ framer animate-width бо зникає в headless, нульові рядки відфільтровано] + маржинальність герой-ризику <40%+ранг; клік→serviceMarginDetail OverviewDetailSheet; розділення FinancesTab/FinancesTabView; 🔴 видалено WaterfallChart.tsx [4-й фейк: вигадував noShowLoss=revenue*0.05]; фікс dummy: displayFin під блюром→skeleton без цифр) · попередній: `924105d6` (M-ANL-04: таб «Склад» editorial-redesign — концепт «склад під контролем»; StockTab→СВІТЛИЙ герой [найтерміновіший розхідник, днів `metric-value` НЕ heading-serif бо Cormorant "18"→"I8", світлофор-tone critical/warning/normal, by-numbers] + col-5 світлофор+список покупок copy/form + col-12 ранг-список за терміновістю; клік→OverviewDetailSheet; розділення StockTab/StockTabView; фікс фейку: dummy-products під блюром→skeleton без цифр; латентний баг M-ANL-02 SourceTab DeltaChip `text-error`→`text-destructive` [`--color-error` не існує в Tailwind v4]) · попередній: `ccebf398` (M-ANL-03: таб «Відгуки» editorial-redesign — концепт «Репутація»; ReviewsTab→ТЕМНИЙ hero-cover [середній бал serif + зірки + NPS спів-герой клік→пояснення + by-numbers композиція] + розподіл-клік→інсайт + стрічка останніх клік→деталі; розділення ReviewsTab/ReviewsTabView; useReviewsMetrics +Δ середній бал+NPS; alternating темний/світлий герой через таб) · попередній: `100b94b2` (M-ANL-02: таб «Джерела» editorial-redesign — концепт «Канали»; SourceTab→світлий featured-герой+гліф-якір+donut support+ранг-список «Інші канали»; розділення SourceTab/SourceTabView; useSourceAttribution +Δ попередній період +completedCount, копійки→гривні; ChannelDonut a11y+Frost-палітра) · попередній: `2c48474f` (M-ANL-01: таб «Огляд» editorial-redesign — Закон темного блоку; OverviewBriefing/OverviewTab/SectionHeading/FeaturedServices|Products|Clients/OverviewDetailSheet/AnalyticsActivation/ClientSheetById + декомпозиція useAnalytics→useAnalyticsMarketing; RevenueLineChart→recharts [isAnimationActive=false для headless]; мертвий ServiceRow видалено) · попередній: `e8837dba` (M-MKT-04 фінал: повний a11y-аудит читабельності story-canvas через a11y MCP — авто-тема accent/pill/badge у ВСІХ режимах, плашка 0.62 [worst-case verified], аватар+ім'я sans+чіп на фото, рукописний+плашка, Champagne+градієнти+стокові ВИДАЛЕНО, нудж власних фото) · попередні: `2d98b3c4` (ітерація-2: образи-пресети + авто-колір + link-zone + Sheet-модалка) · `7b2e72d8` (покроковий story-едітор: рерайт шелла в 5-крок візард + pinned прев'ю + useStoryEditor/storySteps + 5 панелей-кроків + текст/градієнт/сток шаблони; M-MKT-03 +4 палітри; reuse StoryCanvas/storyExport/useStoryData) · попередній: `31557c87` (M-GROW-02 мердж партнерів+альянсів → нова `master_connections` bilateral [kind partner/alliance + role], міграція `20260628000008` additive, 7 споживачів перенацілено, UI PartnersPage один список, латентний RLS-баг публічної сторінки пофікшено `mc_public_read`) · попередній: `3cf3deea` (M-GROW-01 лояльність panel: RPC `get_loyalty_overview`+`get_loyalty_impact`, redemption-tracking, OverviewCard+ProgramProgress)
> 
> **⚡ Sprint-05 Status (звірка 2026-07-12): ФІЧІ ЗАКРИТО — 83/86 ✅ · 3 ↩️ (M-DASH-11 + M-MKT-01/02) = усі 86.** M-SHOP-05 MVP зроблено, ключ NP у Vercel env. **Відкрите лише в тест-блоці: `TEST-M3` (P1) + `TEST-M4` (P1).** `SEC-01` (P0, e2e-сідер міг витерти прод) — ✅ закрито 2026-07-12, guard перевірено наживо · **✅ C-DESK-01 ПОВНІСТЮ ЗАКРИТО (2026-07-06, НЕ задеплоєно): десктоп-лейаут клієнт-зони, sidebar-навігація, усі 8 сторінок** — Фаза 1 шелл `4b6d4d69` · bookings master-detail · messages 2-pane `9386a9d7` · masters галерея `ac940ef6` · loyalty+emoji-purge `f5795c15` · notifications 2-кол `304141b6` · profile sticky-2-кол `d9f8d555` · explore+shop widening `c3481f38`. Наскрізний закон: emoji-заглушки аватара → монограм (спільний MasterAvatar/MediaCover). | Аналітика 7/7 ✅ · Settings M-SET-01..05 ✅ · M-BILL-01 ✅ ЗАДЕПЛОЄНО | Next: `M-SHOP-05` | Трекер: `XDEV/PLANS/SPRINT-05-BACKLOG/TRACKER.md`
...
| `/dashboard/analytics` | Аналітика Pro v2.2 (Editorial Bento, Закон темного блоку): таб «Огляд» editorial-redesign + Bento, MoM, PNG/SVG експорт; виручка, когорти, бізнес-здоров'я, фінанси, склад, розумна націнка, брифінг, CSV | `analytics/page.tsx` | — | `master/analytics/AnalyticsPage.tsx` (включає `MorningBriefing.tsx`, `BusinessHealthScoreWidget.tsx`, `SmartPricingOptimizer.tsx`; **Огляд M-ANL-01:** `sections/OverviewTab.tsx` (+`SectionHeading` спільний), `sections/OverviewBriefing.tsx` (темна editorial-обкладинка герой+by-numbers), `sections/FeaturedServices|Products|Clients.tsx` (герой+ранг-список), `sections/OverviewDetailSheet.tsx` (адаптивний Sheet деталей, payload hero/rows/note/cta), `sections/AnalyticsActivation.tsx` (empty-state), `ClientSheetById.tsx`; `charts/RevenueLineChart.tsx` (recharts ComposedChart, `isAnimationActive=false`); хуки `useAnalytics` + `useAnalyticsMarketing` (декомпозиція); та вкладки: **`SourceTab.tsx` M-ANL-02 ✅** (концепт «Канали»: світлий featured-герой `SourceTabView` + гліф-якір + `ChannelDonut` support [a11y+Frost-палітра] + ранг-список «Інші канали» клік→`OverviewDetailSheet`; хук `useSourceAttribution` +Δ попередній період +completedCount), **`GrowthTab.tsx` M-ANL-07 ✅** (концепт «двигуни росту»: темний герой `GrowthTabView` — додатковий дохід розумних цін +₴N домінанта [fallback uplift=0→LTV+нудж] + LTV Парето featured [гістограма static+winback клік→профіль] + Канали залучення диференційовані рядки + Спрацювання цін/Що бронюють разом смугами + Когорти reuse CohortHeatmap; чесні лейбли «Постійні клієнти/візити» НЕ «бали»; static bars НЕ framer; дані пропсами з page-level extras+useAnalyticsMarketing; видалено 8 мертвих файлів), **`ReviewsTab.tsx` M-ANL-03 ✅** (концепт «Репутація»: темний hero-cover `ReviewsTabView` — середній бал serif + зірки + NPS спів-герой клік→пояснення + by-numbers композиція + розподіл-клік→інсайт + стрічка останніх клік→деталі; хук `useReviewsMetrics` +Δ середній бал+NPS), **`StockTab.tsx` M-ANL-04 ✅** (концепт «склад під контролем»: світлий герой `StockTabView` — найтерміновіший розхідник, днів `metric-value` [НЕ heading-serif: Cormorant "18"→"I8"], світлофор-tone critical/warning/normal + col-5 світлофор+список покупок copy/form + col-12 ранг-список за терміновістю; клік→`OverviewDetailSheet`; empty Pro→skeleton без цифр), **`FinancesTab.tsx` M-ANL-05 ✅** (концепт «P&L звіт»: темний герой `FinancesTabView` — чистий прибуток domінанта + Δ до минулого періоду [Pro, 2-й виклик хука з `previousWindow`] + by-numbers; чесний каскад руху грошей [hairline-смуги СТАТИЧНОЇ ширини, нульові рядки відфільтровано]; маржинальність герой-ризику <40%+ранг, клік→`serviceMarginDetail`; 🔴 `WaterfallChart.tsx` ВИДАЛЕНО [4-й фейк noShowLoss=revenue*0.05]; empty Pro→skeleton без цифр), **`BehaviorTab.tsx` M-ANL-06 ✅** (концепт «ритм бізнесу»: світлий герой `BehaviorTabView` — теплова карта `HeatmapGrid` col-7 + пік-інсайт домінанта + No-Show журнал клік→профіль + Lead time смуги СТАТИЧНОЇ ширини + Vacation чесна врізка→`/settings#vacations`; 🔴 фейк «Рятувальник» прибрано; консолідація 3 під-табів+heatmap, `NoShowTab`/`LeadTimeTab`/`VacationTab` ВИДАЛЕНО; Pro-gate value-екран; fetch: `useLeadTimeDistribution`+`useNoShowMetrics`+`useVacationImpact`, heatmap з `occupancy_heatmap` props) — **усі 7 табів редизайнено (M-ANL-01..07 ✅), Analytics ПОВНІСТЮ закрита; alternating темний/світлий герой через таб; видалені застарілі: WaterfallChart, NoShowTab, LeadTimeTab, VacationTab, BentoSecondary, GrowthLists, DynamicPricingUplift, FlashDealsCard, BroadcastEngagement, LtvConcentration, ServicePairing, LtvHistogram**) |
...
| `/dashboard/billing` | Підписки Monobank: tier, оплата, checkout | `billing/page.tsx` | `billing/actions.ts` | `master/billing/BillingPage.tsx` (M-BILL-01: state-aware герой Starter=офер Pro/Pro=керування col-8 slate + Monobank-панель col-4 чорна з офіц. SVG `public/monobank-logo.svg` + плани 2 тихі рядки; Закон темного блоку; actions недоторкані) |
...
- 🔴 **[EXPLORE-01 2026-07-13]** `explore/data.ts` і `src/app/sitemap.ts` фільтрують `services!inner` + `.eq('services.is_active', true)` — у каталог і sitemap потрапляють ЛИШЕ майстри з ≥1 активною послугою. Причина: онбординг (`dashboard/onboarding/actions.ts:66`) ставить `is_published=true` **до** створення послуг, тож у вітрині висіло 5 порожніх профілів із 10 (2 — тестові, «C2C Test Master»; знято з публікації на проді). Особисте посилання `/<slug>` НЕ зачеплене — майстер шерить себе з першої хвилини. ⚠️ Знайдено пост-деплой смоуком по живому проду, не тестами
...
- `src/app/my/masters/` → мої майстри; дані з `getMyMasters()` (`lib/actions/myMasters.ts`) — спільне джерело (bookings-групування по майстру), реюзиться рейкою в /my/messages. **C-DESK-01 Фаза 2 (десктоп, commit `ac940ef6`):** `MyMastersPage` = мобільний 2-col грід `lg:hidden` (недоторканий) + `DesktopMasters` (`hidden lg:block`) — темний `ClientPageHero` band + featured «Твій головний майстер» (most-visits, tiebreak recency; горизонтальна картка) + «Інші майстри» auto-fit грід `minmax(258px,1fr)`. No-photo → монограм-обкладинка (reuse `monogramHue`/`initialOf`/`CORMORANT` з `public/explore/shared.ts`), НЕ emoji. `/my/masters` у `isWideDesktopRoute` (my/layout.tsx)
- `src/app/my/loyalty/` → `MyLoyaltyPage.tsx` — прогрес лояльності + реферали (таби Лояльність/Реферали, sub-таби C2C «для подруг»/C2B «запросити майстра», промокоди). **C-DESK-01 Фаза 2 (десктоп + emoji-purge, commit `f5795c15`):** усі avatar-заглушки emoji → монограм через спільний `MasterAvatar` (reuse `monogramHue`/`initialOf`/`CORMORANT` з `public/explore/shared.ts`) — 🔴 ЖОДНИХ emoji-fallback. Десктоп-адаптив: `lg:max-w-5xl` центр, таби `lg:max-w-md`, гріди програм/C2C `sm:grid-cols-2`, C2B `lg:max-w-2xl`. `/my/loyalty` у `isWideDesktopRoute`
- `src/app/my/notifications/` → `ClientNotificationsPage.tsx` — in-app нотифікації + pending portfolio consent requests. **C-DESK-01 Фаза 2 (десктоп, commit `304141b6`):** читацька поверхня — mobile `lg:hidden` 1-кол + desktop `hidden lg:block` max-w-4xl 2-кол `grid-cols-[220px_1fr]` (ліва sticky summary «Огляд» + права вузький фід ≤75ch). Під-компоненти `ConsentCard`/`NotifRow`/`FeedGroups`/`EmptyNotifs` переюзані. `/my/notifications` у `isWideDesktopRoute`
- `src/app/my/profile/` → `MyProfilePage.tsx` — форма профілю (аватар/контакти/соцмережі/здоров'я/тема/сповіщення/акаунт). **C-DESK-01 Фаза 2 (десктоп, commit `d9f8d555`):** 2-кол на тому ж дереві (форма-стейт спільний) — root `lg:grid grid-cols-[320px_1fr] max-w-5xl`, identity `editorial-cover` `lg:sticky` ліворуч + форма-секції в праву колонку (мобільний DOM-порядок збережено); save-FAB `lg:left-[264px]`. `/my/profile` у `isWideDesktopRoute`
- `src/app/my/messages/` → мобільний `MessagesListPage.tsx` (`lg:hidden`, спільний клієнт+майстер) — список DM-розмов (T-chat, migration 20260615000002); hairline під тайтлом; **C-MSG-02:** опційний проп `masters?` рендерить `MastersRail.tsx` (рейка «Написати майстру» — майстри БЕЗ активної розмови, дедуп `master.id`===`participant.id`, тап→`?to=id`; реюз `ScrollStrip`; master-safe: майстер проп не передає). Дані рейки = `getMyMasters()` мінус учасники наявних conversations. **C-DESK-01 Фаза 2 (десктоп 2-pane, commit `9386a9d7`):** `src/components/client/MyMessagesDesktop.tsx` (`hidden lg:block`, ТІЛЬКИ клієнт) — ліва інбокс-колонка (Підтримка + розмови + «Написати майстру») + права панель за client-state (`null`→editorial empty · `dm`→`DirectChatPage inPane` · `support`→`SupportChatPage inPane`); pane-swap без route (вибір у `?c=<convId>`|`support`, deep-link); keyed motion.div remount=fresh realtime; «написати майстру»→`getOrCreateConversation`→select→`router.refresh()`. Page рендерить обидва (mobile `lg:hidden` + desktop). `my/layout.tsx` route-aware ширина (див. нижче)
...
- `useChatViewport(enabled=true)` (`src/lib/hooks/useChatViewport.ts`) — висота = `window.visualViewport.height` (resize+scroll), `scrollTo(0,0)` проти iOS-зсуву, lock body overflow/overscroll; fallback innerHeight / CSS `100dvh` до гідратації. **`enabled=false` (C-DESK-01):** side-effect вимкнено — contained-режим НЕ лочить body-scroll на десктопі.
- `ChatShell.tsx` — за замовч. корінь `position: fixed; inset-x-0 top-0 z-40` + height від хука. **Fixed навмисно** — імунітет до скролу сторінки й padding батьків-layout-ів, тож `ChatHeader` — єдине джерело safe-area-top (без подвійного відступу). **`contained` (C-DESK-01):** fill-pane `relative h-full` замість fixed + `useChatViewport(false)` — для десктоп 2-pane. Default `false` = мобільний/майстер byte-identical.
...
- `types.ts` — `ChatMessage { id, sender_id, message, attachment_url, attachment_width?, attachment_height?, attachment_blur?, created_at, read_at? }`. Розміри вимірює `lib/upload/imageMeta.ts` (`buildChatAttachment`) у браузері перед upload; є → `next/image` з blur, немає (legacy) → raw `<img>`.
- **Консумери:** `DirectChatPage` (DM клієнт+майстер) + `SupportChatPage` (support клієнт+майстер) = повний ChatShell; `AdminSupportConsole` (десктоп-пульт) переюзає ЛИШЕ `ChatMessageList` (currentUserId=adminId з `admin/support/page.tsx`), лишає свій каркас черги+resolve. **`inPane`-проп (C-DESK-01):** `DirectChatPage`/`SupportChatPage` прокидають `contained` у ChatShell + ховають back-кнопку — для десктоп 2-pane клієнта; default off = майстер/мобільний недоторкані.
- **Layout bypass:** `my/layout.tsx` + `DashboardLayout.tsx` `isChatRoute` матчить і `/messages/[id]` (regex, мобільний fullscreen тред), не тільки support/chat. **C-DESK-01 route-aware ширина (`my/layout.tsx`):** `/my/bookings`+`/my/messages` на lg → повна ширина (`lg:max-w-none lg:px-0`), інакше `max-w-lg` (латентний squeeze master-detail у `max-w-lg` полагоджено — `pl-264px` офсету самого НЕ достатньо). `/my/messages` додатково full-height focus: `lg:pt-0` (без phantom `md:pt-20`) + `SupportWidget`/`ChannelBanner` в `lg:hidden`-обгортці (support уже в pane). Мобільний контейнер незмінний.
- **M-HELP-02 (support comms + інбокс-nav, commit `aa7944f0`):** `supportHours.ts` `getSupportPresence()` (щодня 8:00-20:00 → presence-лейбл/крапка); `SupportPage` картка активної розмови (статус+«Нова відповідь»=похідна `sender_id`, БЕЗ read-таблиці); `SupportChatPage` статус-чіп у хедері. **Єдиний інбокс:** `messages.ts getInboxSummary()` + `support.ts getSupportChatState()` (бейдж = DM-непрочитане + відповідь підтримки, БЕЗ нової DB); `InboxNavButton` (спільний icon/fab/row, live realtime `direct_messages`/`support_messages`+focus, канал `useId`); `MessagesListPage` закріплений рядок «Підтримка BookIT». **Кнопка «Чат» у nav:** майстер `DashboardTopBar`(icon)+`MobileHub`(hub-row «Спілкування»); клієнт `PublicNavbar`(desktop icon)+support-віджет. 🔴 `C-MSG-01` закрито через M-CHAT-01.
- **C-MSG-03 (нова розмова, commit `e33faff2`):** «+» у хедері `MessagesListPage` → `NewConversationSheet` (адаптивний пікер role-aware). Майстер: клієнти з акаунтом→діалог + без-акаунтні→запросити (share/Telegram); клієнт: майстри з історії bookings→діалог; адмін: `AdminSupportConsole` таби Майстри/Клієнти→`createAdminTicketForUser`. Server: `getMessageableContacts`+`createAdminTicketForUser`+`getAdminMessageTargets`. 🔴🔴 **Для роботи з майстрами/клієнтами:** (1) клієнти майстра = RPC **`get_master_clients(p_master_id)`** (bookings-агрегація), НЕ `client_master_relations` (лише VIP/tagged підмножина); (2) **`master_profiles` PK = `id` == profiles.id == auth uid; колонки `user_id` НЕ ІСНУЄ** — селект `master_profiles.user_id` = тихий null-баг; детект ролі → `profiles.role`. Дубль плаваючий inbox-fab прибрано.
...
- `src/components/public/ShopPage.tsx` — каталог: фільтр-чіпи + грід тайлів (тайл = `<Link>` на сторінку товару), кошик з `useShopCart()`, рендерить `ShopCartBar`. **C-DESK-01 Фаза 2 (десктоп widening, commit `c3481f38`):** контейнер `max-w-lg → lg:max-w-5xl`, грід `+sm:grid-cols-3 lg:grid-cols-4`; cart-піл `.shop-cart-bar` + globals `body.has-client-shell .shop-cart-bar{left:264px}` (authed only, guest недоторканий) — checkout-логіка не чіпана
...
- **`src/components/public/shop/ShopCartBar.tsx`** (M-SHOP-03, NEW) — sticky cart-кнопка + `CartDrawer` (checkout pickup/Nova Poshta) + `OrderSuccess` (fixed-overlay). Читає контекст; на каталозі І сторінці товару (активна одна за раз). **M-SHOP-05 (commit `1cf2746d`):** NP-доставка = `NovaPoshtaPicker` (пошук міста autocomplete → вибір відділення/поштомату) замість ручного textarea; order зберігає `np_city_ref/name`+`np_warehouse_ref/name` + readable `delivery_address`
- **Nova Poshta інтеграція (M-SHOP-05, MVP):** `src/lib/novaposhta/client.ts` (server-only NP API-клієнт — `getCities`+`getWarehouses`, read-only, ключ `NOVAPOSHTA_API_KEY` у `.env.local`, `next.revalidate:3600`) · `src/lib/actions/novaPoshta.ts` (server actions `searchNpCities`/`getNpWarehouses`) · `src/components/public/shop/NovaPoshtaPicker.tsx` (клієнтський picker: місто debounce 300 → відділення). 🔴 ТТН/трек/розрахунок = наступна фаза (потребує sender-профілю майстра: NP counterparty, CitySender, адреса відправлення, платник). Endpoint: `https://api.novaposhta.ua/v2.0/json/`
...
| `direct_messages` | Повідомлення DM: `conversation_id` FK, `sender_id` FK, `message`, `attachment_url` + `attachment_width/height/blur`, `read_at`. RLS via conversations. Вкладення → bucket `support_attachments`, шлях `dm/<conversation_id>/…` (політики: міграція 20260710000001) |
...
| `support_messages` | Повідомлення підтримки: `ticket_id` FK, `sender_id` FK, `message`, `attachment_url` (скріншоти) + `attachment_width/height/blur`. Шлях у bucket: `<ticket_id>/…` |
...
| `get_analytics_extras` | Об'єднаний мега-RPC аналітики (зайнятість, когорти, LTV, аномалії, ROI Smart Pricing та зв'язки послуг) — міграція 20260605000000. **🔴 OPT-DB-03 фікс (20260712000001):** `scope` `'finances'` і `'stock'` РОЗДІЛЕНО — раніше спільна умова `IN ('all','finances','stock')` рахувала обидві секції для КОЖНОГО scope, тож `StockTab` дарма ганяв найважчий `get_finance_analytics` (прод: stock 9.7мс/2618Б → 3.6мс/723Б). ⚠️ `p_scope text DEFAULT 'all'` — дефолт зберігати 1:1 при будь-якому `CREATE OR REPLACE` (Postgres не вміє прибирати дефолти). Хук: `useAnalyticsExtras.ts` (scope у queryKey) |
| `get_week_new_client_phones` | **NEW (OPT-DB-04, 20260712000002).** Повертає `text[]` нормалізованих телефонів клієнтів, які цього тижня прийшли ВПЕРШЕ (set-diff через `NOT EXISTS`). Замінив fetch 5000 історичних телефонів + діф у JS: `.limit(5000)` **без `ORDER BY`** тихо робив число «нових клієнтів» хибним за межею. Нормалізація 1:1 з клієнтською (`replace(/\D/g,'')`) — `StatsModals` звіряє через `newPhones.has(norm(...))`. SECURITY DEFINER + `auth.uid()` guard (42501), anon без EXECUTE. Індекс не потрібен: покриває `idx_bookings_master_status_date`. Споживач: `useDashboardStats.ts` |
...
- `20260712000004_bookings_no_overlap.sql` — 🔴 **закрито баг «двоє клієнтів на один час».** `EXCLUDE USING gist (master_id WITH =, tsrange(date+start_time, date+end_time, '[)') WITH &&) WHERE (status <> 'cancelled' AND date >= DATE '2026-07-13')` + `btree_gist`. **Root cause:** `rescheduleBooking` робив `UPDATE` часу **без жодної перевірки перекриття**, а unique-індекс `booking_slot_collision` ловить **лише точний збіг `start_time`** — тож `09:15-10:05` і `10:00-10:50` проходили обидві (на проді знайдено 15 перекритих пар, 2 — живі клієнти). Констрейнт закриває `createBooking`, `rescheduleBooking`, прямий REST і race одразу. ⚠️ Діапазон `'[)'` — щоб **суміжні** записи лишались дозволені; предикат із **літеральною** датою — бо 30 історичних рядків порушують правило, а EXCLUDE не підтримує `NOT VALID`. Код обробляє `23P01`. ✅ застосовано на прод 2026-07-12 (`86ac9b2e`)
