...
| `/dashboard/marketing` | Marketing Hub: Story Generator (M-MKT-04 покроковий 5-крок візард) + Broadcast розсилки (in-app/Push/Telegram/SMS) | `marketing/page.tsx` | `marketing/actions.ts` | `master/marketing/StoryGenerator.tsx` (тонкий оркестратор), `BroadcastEditor.tsx`, `BroadcastHistory.tsx`. **Story-едітор (M-MKT-04):** кроки Тип→Контент→Вигляд→Стиль→Готово, завжди-pinned live-прев'ю (mobile зверху / desktop sticky праворуч). Мозок = `story/useStoryEditor.ts` (стейт+преміум blur-gating+навігація+взаємовиключні фон-джерела), модель = `story/storySteps.ts`. UI: `story/StoryPreview.tsx`, `story/StepNav.tsx`, `story/steps/{StepType,StepContent,StepLook,StepStyle,StepExport}.tsx`. Reuse-фундамент (НЕ переписано): `story/StoryCanvas.tsx` (+адитивний градієнт-шар `bgGradientCss`), `story/storyExport.ts`, `story/useStoryData.ts`. Шаблони (`storyConstants.ts`): `TEXT_TEMPLATES` (текст-пресети по-режимах) + `BG_GRADIENTS` (6 CSS-градієнтів) + `STOCK_PHOTOS` (порожній слот `public/story-bg/`, секція схована до ассетів). Палітр 10 (M-MKT-03 +terracotta/lavender/forest/champagne). 32 unit-тести. Експорт/TMA/преміум-гейтинг 1:1 |
