...
> Оновлено: 2026-06-28 · Джерело: живий код (v9.0.0) · Останній код-коміт: `3cf3deea` (M-GROW-01 лояльність panel: RPC `get_loyalty_overview`+`get_loyalty_impact` [auth.uid, REVOKE anon], `bookings.loyalty_label/amount` redemption-tracking forward-only, `createBooking` пише знижку лояльності, `useLoyaltyStats`, `LoyaltyPage` OverviewCard+ProgramProgress, ClientsPage `?loyaltyMin/loyaltyExact` фільтр) · попередній: `255bbcf3` (M-REV-02 авто-flash A+B+C + bug тогл · M-REV-03 статистика акцій flash_deal_recipients)
> 
> **⚡ Sprint-05 Status (ACTIVE):** 42/78 ✅ · 1 ↩️ (M-DASH-11 скасовано) · **Фаза 3: Revenue 6/17 done · Growth почато (M-GROW-01 ✅)** | Next: `M-GROW-02` — об'єднати реферали+партнери (HARD) | Трекер: `XDEV/PLANS/SPRINT-05-BACKLOG/TRACKER.md`
