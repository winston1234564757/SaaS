...
1. **`ServiceEditor`** (Сторінки `/dashboard/services/new` та `/dashboard/services/[id]`)
   - *Формат:* Повносторінковий редактор (No-Modals policy).
   - *Функція:* Створення та редагування послуги.
2. **`ProductEditor`** (Сторінки `/dashboard/products/new` та `/dashboard/products/[id]`)
   - *Формат:* Повносторінковий редактор (No-Modals policy).
   - *Функція:* Створення та редагування товарів.
3. **`RestockDrawer`** (`RestockDrawer.tsx`)
   - *Формат:* `BottomSheet`
   - *Функція:* Швидке поповнення залишків товару на складі.
