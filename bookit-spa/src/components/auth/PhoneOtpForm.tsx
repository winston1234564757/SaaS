...
async function claimMasterRole(phone: string): Promise<{ error: string | null }> {
  ...
  const { error: profileError } = await supabase
    .from('profiles')
    .upsert({ id: user.id, role: 'master', phone }, { onConflict: 'id', ignoreDuplicates: false });
  ...
  const { error: masterError } = await supabase
    ...
    .upsert({ id: user.id, slug, is_published: false }, { onConflict: 'id', ignoreDuplicates: true });
...
const processRegistrationReferral = async (_userId: string, _refCode: string): Promise<void> => {};
  ...
  async function handleVerifyOtp() {
    ...
    if (selectedRole === 'master') {
      ...
      if (intendedPlan === 'pro' || intendedPlan === 'studio') {
        navigate(`/dashboard/billing?plan=${intendedPlan}`);
      } else if (data.isNew) {
          ...
          {step === 'phone' && (
            ...
            >
              ...
              <p className="text-center text-sm text-[#6B5750] mt-5">
                {isLogin ? (
                  <>Ще не зареєстровані?{' '}
                    <Link to="/register" className="text-[#789A99] font-medium hover:underline">Створити акаунт</Link>
                  </>
                ) : (
                  <>Вже є акаунт?{' '}
                    <Link to="/login" className="text-[#789A99] font-medium hover:underline">Увійти</Link>
                  </>
                )}
              </p>
            </motion.div>
