...
import { isChurned, getAutoTags } from './ClientsPage';
...
export function ClientDetailSheet({ client, onClose, onVipChange }: ClientDetailSheetProps) {
  ...
  useEffect(() => { setNoteText(savedNote); }, [savedNote, client?.id]);
  useEffect(() => { setReminderResult(null); setNoteSaved(false); }, [client?.id]);
  ...
  async function handleSaveNote(text = noteText) {
    if (!client?.client_phone || !masterProfile?.id) return;
    setNoteSaving(true);
    const { error } = await supabase
      .from('master_client_notes')
      .upsert({ master_id: masterProfile.id, client_phone: client.client_phone, note_text: text }, { onConflict: 'master_id,client_phone' });
    if (!error) {
      setNoteSaved(true);
  ...
  async function handleToggleVip() {
    ...
    await supabase.from('client_master_relations').update({ is_vip: newVip }).eq('id', client.relation_id);
    ...
    <AnimatePresence>
      {client && (
        <>
          ...
          >
            <div className="rounded-t-[28px] border border-white/60 overflow-hidden"
              style={{ background: 'rgba(255,248,244,0.96)', backdropFilter: 'blur(24px)', boxShadow: '0 -8px 40px rgba(44,26,20,0.14)' }}
            >
              <div className="flex justify-center pt-3 pb-1"><div className="w-9 h-1 rounded-full bg-[#D4C5BE]" /></div>
              ...
              <div className="flex items-center gap-3 px-5 pt-2 pb-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {client.is_vip && <span className="text-[10px] font-bold text-[#D4935A] bg-[#D4935A]/12 px-1.5 py-0.5 rounded-full flex-shrink-0">VIP</span>}
                  </div>
                  <a href={`tel:${client.client_phone}`} className="flex items-center gap-1 text-xs text-[#789A99] hover:text-[#5C7E7D] transition-colors mt-0.5">
                    <Phone size={11} />{client.client_phone}
                  </a>
                </div>
                <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-2xl bg-white/70 border border-white/80 text-[#A8928D] hover:text-[#6B5750] transition-colors flex-shrink-0">
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="px-5 pb-8 overflow-y-auto max-h-[60vh] flex flex-col gap-4">
                  ...
                  return tags.length > 0 ? (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tags.map(tag => (
                          <span key={tag.label} className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ color: tag.color, background: tag.bg }}>
                            {tag.label}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="space-y-2">
                    <button
                      onClick={async () => {
                        ...
                        // TODO: Implement Telegram churn reminder via API endpoint
                        await new Promise(r => setTimeout(r, 500));
                        setReminderResult('Функція доступна тільки у Pro версії з Telegram');
                        setReminding(false);
                      }}
                      disabled={reminding}
                    ...
                    {reminderResult && (
                      <p className={`text-xs text-center px-2 ${reminderResult.startsWith('✅') ? 'text-green-600' : 'text-[#A8928D]'}`}>{reminderResult}</p>
                    )}
                  </div>
                )}

                {client.relation_id ? (
                  <button onClick={handleToggleVip} disabled={toggling}
                    className={`flex items-center justify-center gap-2 w-full py-3 rounded-2xl text-sm font-semibold transition-all ${client.is_vip ? 'bg-[#D4935A]/12 text-[#D4935A] hover:bg-[#D4935A]/20' : 'bg-white/70 border border-white/80 text-[#6B5750] hover:bg-white'} disabled:opacity-60`}
                  >
                    <Crown size={15} />
                    {client.is_vip ? 'Прибрати VIP статус' : 'Позначити як VIP'}
                  </button>
                ) : (
                  <p className="text-[11px] text-[#A8928D] text-center">VIP доступний для клієнтів з акаунтом Bookit</p>
                )}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => handleSaveNote()} disabled={noteSaving || noteText === savedNote}
                      className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-semibold transition-all ${noteSaved ? 'bg-[#5C9E7A]/12 text-[#5C9E7A]' : 'bg-[#789A99]/10 text-[#789A99] hover:bg-[#789A99]/20 disabled:opacity-40 disabled:cursor-not-allowed'}`}
                    >
                  ...
                  <textarea
                    value={noteText} onChange={e => handleNoteChange(e.target.value)}
                ...
                <div>
                  ...
                  {loading ? (
                    <div className="flex justify-center py-4"><div className="w-5 h-5 rounded-full border-2 border-[#789A99] border-t-transparent animate-spin" /></div>
                  ) : bookings.length === 0 ? (
