...
import { Users, Star, Phone, Calendar, TrendingUp, Loader2, Link2, Zap, LayoutGrid, List, ChevronDown } from 'lucide-react';
...
export function getAutoTags(client: ClientRow): AutoTag[] {
  ...
  if (daysSinceLast !== null) {
    if (daysSinceLast >= 120) tags.push({ label: '💤 Спить', color: '#C05B5B', bg: '#C05B5B15' });
    else if (daysSinceLast >= 60) tags.push({ label: '⚠️ Під ризиком', color: '#D4935A', bg: '#D4935A15' });
  }
  return tags;
}
...
export function ClientsPage() {
  ...
  const filtered = sortClients(
    clients.filter(c => c.client_name.toLowerCase().includes(search.toLowerCase()) || c.client_phone.includes(search)),
    sort,
  );
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
            ...
            <div className="absolute right-0 top-full mt-1.5 z-30 bg-white/95 backdrop-blur-sm rounded-2xl border border-white/80 shadow-lg overflow-hidden min-w-[160px]">
              {SORT_OPTIONS.map(opt => (
                <button key={opt.value} onClick={() => { setParam('sort', opt.value); setSortOpen(false); }}
                  className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${sort === opt.value ? 'bg-[#789A99]/12 text-[#5C7E7D] font-semibold' : 'text-[#6B5750] hover:bg-[#F5E8E3]/60'}`}
                >
                  {opt.label}
                </button>
              ))}
        ...
        <div className="flex rounded-2xl bg-white/70 border border-white/80 overflow-hidden">
          {(['list', 'grid'] as const).map(v => (
            <button key={v} onClick={() => setParam('view', v)}
              className={`px-3 py-3 transition-colors ${view === v ? 'bg-[#789A99]/15 text-[#5C7E7D]' : 'text-[#A8928D] hover:text-[#6B5750]'}`}
            >
      ...
      {isLoading ? (
        ...
        search ? (
          <div className="bento-card p-10 flex flex-col items-center gap-3 text-center">
            <div className="w-14 h-14 rounded-full bg-[#F5E8E3] flex items-center justify-center"><Users size={26} className="text-[#A8928D]" /></div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bento-card p-6 flex flex-col gap-5">
            <div className="text-center">
              <div className="w-16 h-16 rounded-3xl bg-[#789A99]/10 flex items-center justify-center mx-auto mb-4"><Users size={28} className="text-[#789A99]" /></div>
              <p className="text-base font-bold text-[#2C1A14]">Ваша база клієнтів порожня</p>
              <p className="text-sm text-[#A8928D] mt-1 text-balance">Ось як залучити перших клієнтів за 24 години</p>
            </div>
            <div className="flex flex-col gap-3">
              {[
                ...
                { icon: Link2,     color: '#C05B5B', title: 'Додайте посилання в bio', desc: 'Одне посилання в bio Instagram — і клієнт одразу потрапляє до вашого онлайн-розкладу.' },
              ].map((step, i) => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((client, i) => {
            ...
            return (
              <motion.div key={client.id} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.03 }}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <a href={`tel:${client.client_phone}`} className="flex items-center gap-1 text-xs text-[#A8928D] hover:text-[#789A99] transition-colors mt-0.5" onClick={e => e.stopPropagation()}>
                      <Phone size={11} />{client.client_phone}
                    </a>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#F5E8E3]/60">
                  <div><p className="text-[10px] text-[#A8928D]">Візитів</p><p className="text-sm font-bold text-[#2C1A14]">{client.total_visits}</p></div>
                  <div className="text-right"><p className="text-[10px] text-[#A8928D]">Витрачено</p><p className="text-sm font-bold text-[#2C1A14]">{formatPrice(client.total_spent)}</p></div>
                </div>
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {tags.map(tag => <span key={tag.label} className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tag.color, background: tag.bg }}>{tag.label}</span>)}
                  </div>
                )}
              </motion.div>
            );
        ...
        <div className="flex flex-col gap-3">
          {filtered.map((client, i) => (
            <motion.div key={client.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
      ...
      <ClientDetailSheet client={selectedClient} onClose={() => setSelectedClient(null)} onVipChange={handleVipChange} />
    </div>
  );
}
