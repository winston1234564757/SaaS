...
// ── Реалізація getOrCreateReferralLink через Supabase (без Server Actions) ──
...
async function getOrCreateReferralLink(
  ...
  try {
    ...
    // Шукаємо існуючий лінк
    const { data: existing } = await supabase
      .from('referral_links')
      .select('code')
      .eq('owner_id', ownerId)
      .eq('target_type', targetType)
      .is('target_master_id', null)
      .maybeSingle();

    if (existing) {
      return { success: true, link: `${appUrl}/invite/${existing.code}` };
    }
    ...
    return { success: true, link: `${appUrl}/invite/${code}` };
  } catch (e: unknown) {
