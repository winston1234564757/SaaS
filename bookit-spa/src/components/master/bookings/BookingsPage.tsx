...
function startOfWeek(d: Date) { const r = new Date(d); const day = r.getDay(); r.setDate(r.getDate() - (day === 0 ? 6 : day - 1)); return r; }
...
export function BookingsPage() {
  ...
  const { dateFrom, dateTo, label } = useMemo(() => {
    ...
    return { dateFrom: toISO(start), dateTo: toISO(end), label: `${UA_MONTHS[anchor.getMonth()]} ${anchor.getFullYear()}` };
  }, [view, anchor]);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(b => b.client_name.toLowerCase().includes(q) || b.client_phone.includes(q));
    }
    return result;
  }, [bookings, statusFilter, search]);

  useEffect(() => { setVisibleCount(PAGE_SIZE); }, [dateFrom, dateTo, statusFilter, search]);
  ...
  const grouped = useMemo(() => {
    ...
    visibleItems.forEach(b => { const arr = map.get(b.date) ?? []; arr.push(b); map.set(b.date, arr); });
    return map;
  }, [visibleItems, view]);
  ...
  const totalRevenue = filtered.filter(b => b.status === 'completed').reduce((sum, b) => sum + b.total_price, 0);
    ...
    <div className="flex flex-col gap-4 pb-8">
      <div id="tour-bookings-header" className="bento-card p-5">
        ...
        <div className="flex gap-2 mt-4">
          {(['day', 'week', 'month'] as ViewMode[]).map(v => (
            <button key={v} onClick={() => setView(v)}
              className={`flex-1 py-2 rounded-2xl text-xs font-medium transition-all ${view === v ? 'bg-[#789A99] text-white' : 'bg-white/60 text-[#6B5750] hover:bg-white/80'}`}
            >
        ...
        <div className="flex items-center justify-between mt-3">
          <button onClick={() => navigate(-1)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/70 border border-white/80 text-[#6B5750] hover:bg-white transition-colors">
            <ChevronLeft size={16} />
          </button>
          <div className="text-center">
            ...
            {isToday
              ...
              : <button onClick={() => setAnchor(new Date())} className="text-[11px] text-[#789A99] font-medium hover:underline">Повернутись до сьогодні</button>
            }
          </div>
          <button onClick={() => navigate(1)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/70 border border-white/80 text-[#6B5750] hover:bg-white transition-colors">
      ...
      {error && (
        <div className="bento-card p-3 flex items-start gap-2 border border-[#D4935A]/40 bg-[#FFF7F0]">
          <div className="mt-0.5 text-[#D4935A]"><AlertTriangle size={16} /></div>
          <div className="text-xs text-[#6B5750]">
            <p className="font-semibold">Не вдалося завантажити записи.</p>
            <p className="mt-0.5">Перезавантажте сторінку або перевірте підключення до мережі.</p>
          </div>
        </div>
      )}
      ...
      <div id="tour-bookings-filter" className="flex items-center gap-2">
        <div className="flex gap-1.5 overflow-x-auto pb-1 flex-1">
          {STATUS_FILTERS.map(f => (
            <button key={f.value} onClick={() => setStatusFilter(f.value)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${statusFilter === f.value ? 'bg-[#789A99] text-white' : 'bg-white/70 border border-white/80 text-[#6B5750] hover:bg-white'}`}
            >
              {f.label}
            </button>
          ))}
        ...
        {isPro ? (
          <button onClick={() => exportToCsv(filtered, label)} disabled={filtered.length === 0}
          ...
          <button disabled className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-full bg-white/70 border border-white/80 text-xs font-medium text-[#A8928D] cursor-not-allowed" title="Тільки для Pro">
            <Lock size={11} /> CSV
          </button>
        )}
      </div>
      ...
      <AnimatePresence mode="popLayout">
        {isLoading ? (
          <motion.div key="loading" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.2 }}
          ...
          isGenuinelyEmpty ? (
            <motion.div key="empty-share" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.2 }} className="flex flex-col gap-4 w-full">
              <div className="bento-card p-6 flex flex-col items-center gap-3 text-center">
                <div className="w-14 h-14 rounded-3xl bg-[#789A99]/10 flex items-center justify-center"><CalendarDays size={26} className="text-[#789A99]" /></div>
            ...
            <motion.div key="empty" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.2 }}
              className="bento-card p-10 flex flex-col items-center gap-3 text-center w-full"
            >
              <div className="w-14 h-14 rounded-full bg-[#F5E8E3] flex items-center justify-center"><CalendarDays size={26} className="text-[#A8928D]" /></div>
          ...
          <motion.div key="day-list" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.2 }} className="flex flex-col gap-3 w-full">
            {visibleItems.map((b, i) => <Suspense key={b.id}><BookingCard booking={b} index={i} /></Suspense>)}
            {hasMore && (
              <button onClick={() => setVisibleCount(c => c + PAGE_SIZE)} className="w-full py-3 rounded-2xl bg-white/70 border border-white/80 text-sm font-medium text-[#789A99] hover:bg-white transition-colors">
                Показати ще ({filtered.length - visibleCount})
              </button>
            )}
          </motion.div>
        ) : (
          <motion.div key="grouped-list" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.2 }} className="flex flex-col gap-4 w-full">
              ...
              return (
                <div key={date}>
                  <div className="flex items-center gap-2 mb-2 px-1">
                    <p className="text-xs font-semibold text-[#6B5750] capitalize">{dayName}, {d.getDate()} {month}</p>
                    <div className="flex-1 h-px bg-[#F5E8E3]" />
                    <span className="text-[11px] text-[#A8928D]">{items.length} зап.</span>
                  </div>
                  <div className="flex flex-col gap-3">
                    {items.map((b, i) => <Suspense key={b.id}><BookingCard booking={b} index={i} /></Suspense>)}
                  </div>
                </div>
              );
            })}
      ...
      <ManualBookingForm isOpen={formOpen} onClose={() => setFormOpen(false)} />
    </div>
  );
}
