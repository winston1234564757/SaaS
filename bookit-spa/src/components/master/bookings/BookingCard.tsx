...
// TODO: Implement via API endpoint (was server action in Next.js)
async function notifyClientOnStatusChange(_bookingId: string, _status: string): Promise<void> {}
    ...
    >
      ...
      {booking.status === 'pending' && (
        <div className="flex gap-2 px-4 pb-3" onClick={e => e.stopPropagation()}>
          ...
          >
            {confirmMutation.isPending ? <Loader2 size={11} className="animate-spin" /> : <Check size={11} />}
            Підтвердити
          </button>
          <button
          ...
          >
            {cancelMutation.isPending ? <Loader2 size={11} className="animate-spin" /> : <X size={11} />}
            Скасувати
          </button>
        </div>
      )}
