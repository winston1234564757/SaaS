...
function ClientSheetById({ clientId, masterId, clientName, onClose }: {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('bookings').select('client_phone, date, total_price, status, client_name')
        .eq('master_id', masterId).eq('client_id', clientId).order('date', { ascending: false }),
      supabase.from('client_master_relations').select('id, is_vip')
      ...
      <div className={cn(
        ...
        {isLockedDateRange && (
          <div className="mt-3 flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl bg-[#789A99]/8 border border-[#789A99]/20">
            ...
            <Link to="/dashboard/billing" className="text-[11px] font-bold text-[#789A99] whitespace-nowrap">
              Оновити →
            </Link>
          </div>
        )}
            ...
            {[
              ...
              <div key={step.title} className="flex gap-3 p-4 rounded-2xl bg-white/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Link to={step.href} className="inline-flex mt-1.5 text-xs font-semibold text-[#789A99] hover:text-[#5C7E7D] transition-colors">
                    {step.cta}
                  </Link>
                </div>
              </div>
