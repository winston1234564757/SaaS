...
import { joinStudio } from '@/lib/actions/studio';
...
export function StudioJoinPage({ studio, token }: Props) {
  ...
  useEffect(() => {
    if (!joined) return;
    const t = setTimeout(() => navigate('/dashboard/studio'), 2000);
      ...
      >
        {joined ? (
          ...
          <>
            ...
            <Link to="/dashboard" className="text-sm text-[#789A99] font-medium hover:underline">
              До кабінету →
            </Link>
          </>
        ) : (
          <>
            ...
            <Link to="/dashboard" className="text-xs text-[#A8928D] hover:text-[#6B5750] transition-colors">
              Скасувати
            </Link>
          </>
        )}
