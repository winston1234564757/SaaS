...
старому ноуті (користувач `Vitossik`) велася сесія «Migration»: ми переносили робоче
...
- Ім'я користувача змінилось `Vitossik → Vitos` — прибиті шляхи треба виправити (`fix-paths.py`).
...
### 3. Виправ прибиті шляхи (Vitossik → Vitos)
```powershell
