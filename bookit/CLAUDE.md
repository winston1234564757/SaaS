...
## 🚨 DATA-PIPELINE FIRST PROTOCOL (PERMANENT LAW)

### NO GHOST UI — Перевірка Data Flow перед будь-яким UI

**НІКОЛИ** не планувати і не реалізовувати client-facing UI для відображення даних без верифікації повного data pipeline. Перед рендером будь-яких даних (локація, ціна, badge, статус) — перевірити ВСІ три шари:

| Шар | Питання | Як перевірити |
|-----|---------|---------------|
| **DB Layer** | Чи є колонка у таблиці? | `grep` міграції або `select` query |
| **Input Layer** | Чи є форма/UI для майстра де ввести це значення? | Прочитати SettingsPage або відповідний CRUD компонент |
| **Mutation Layer** | Чи є Server Action / `handleSave` що зберігає це поле? | Перевірити `.update({...})` виклики |

**Якщо ХОЧА Б ОДИН шар відсутній → СПОЧАТКУ додати відсутній шар, ПОТІМ UI.**

### FILE EXISTENCE ≠ COMPLETION

**НІКОЛИ** не помічати задачу як `✅ skip` або `done` тільки тому що файл (`webhook.ts`, `SettingsPage.tsx`, `BottomSheet.tsx`) існує. Потрібно **прочитати файл** і переконатись що він відповідає конкретній enterprise-grade вимозі.

### DB-TO-DOM THINKING

Проектувати фічі знизу вгору: **DB Schema → Server Action → Form Input → UI Component**. Ніколи не починати з UI без розуміння як стан персистується.
