-- Migration 078: RPC for retention-based rebooking reminders
-- Returns (master_id, client_id, master_tg_chat) for clients whose last completed
-- visit was exactly retention_cycle_days ago, with no future bookings (anti-spam),
-- and no reminder already sent today (idempotency).
...
AS $$
  ...
  last_visits AS (
    SELECT b.master_id, b.client_id, MAX(b.date::date) AS last_date
    FROM bookings b
  ...
  due AS (
    ...
    WHERE lv.last_date = p_today - mc.cycle * INTERVAL '1 day'
  ),
  has_future AS (
