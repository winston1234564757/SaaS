...
const BEAUTY_PHRASES = [
  'Шукаємо вільне віконечко...',
  'Малюємо ідеальні стрілки...',
...
export function BeautyLoader({ message }: BeautyLoaderProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-peach overflow-hidden">
      {/* Background Blobs for Atmosphere */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-sage/10 rounded-full blur-[80px]" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full blur-[100px]" style={{ background: 'color-mix(in srgb, var(--accent-on) 20%, transparent)' }} />
      </div>
      ...
      <div className="relative flex flex-col items-center">
        ...
        <div className="mt-12 w-32 h-1 rounded-full overflow-hidden" style={{ background: 'color-mix(in srgb, var(--accent-on) 30%, transparent)' }}>
          <motion.div
            initial={{ x: '-100%' }}
            animate={{ x: '100%' }}
            transition={{ 
