...
export function PromoTemplates({ slug, defaultOpen = false, plan, onUpgrade }: PromoTemplatesProps) {
  ...
  const freeTemplates = [
    ...
    },
    {
      label: 'Bio',
      badge: 'Профіль',
      text: `Онлайн-запис 24/7 — переходьте за посиланням:\n${shortUrl}`,
      isPro: false,
    },
    {
              ...
              {freeTemplates.map((t, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: locked ? 'color-mix(in srgb, var(--accent-on) 45%, transparent)' : 'color-mix(in srgb, var(--accent-on) 70%, transparent)',
                      border: locked ? '1px solid color-mix(in srgb, var(--accent) 25%, transparent)' : '1px solid color-mix(in srgb, var(--accent-on) 85%, transparent)',
                    }}
                  >
                    <div className="flex items-center justify-between px-3 pt-2.5 pb-1.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span
                          ...
                          style={{ background: 'color-mix(in srgb, var(--sage) 12%, transparent)', color: 'var(--sage)' }}
                        >
                          {t.badge}
                        </span>
                        {t.isPro && (
