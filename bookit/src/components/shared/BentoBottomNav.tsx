    ...
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[75] px-4 pb-safe-bottom pb-4 pointer-events-none">
        <div className="mx-auto max-w-lg">
          <motion.div 
            layout
            className="bg-secondary/90 backdrop-blur-3xl border border-border shadow-lg rounded-xl p-2 flex items-center justify-between pointer-events-auto relative"
          >
              ...
              <button type="button"
                ...
                className={cn(
                  ...
                  isOpen 
                    ...
                    : isBentoActive 
                      ? "bg-sage text-white" 
                      : "bg-secondary text-text-main border-2 border-peach/15"
                )}
              >
                ...
                {!isOpen && isBentoActive && (
                  <div className="absolute inset-0 animate-pulse" style={{ background: 'color-mix(in srgb, var(--accent-on) 20%, transparent)' }} />
                )}
              </button>
            </div>
  ...
  return (
    ...
    >
      <Link
        href={href}
        className={cn(
          "w-full h-full rounded-xl p-5 flex transition-all duration-500 active:scale-95 border relative overflow-hidden shadow-sm",
          ...
          isActive 
            ...
            : "bg-gradient-to-br from-secondary via-secondary/90 to-accent/5 text-text-main border-border hover:border-accent/40 shadow-sm"
        )}
      >
        ...
        <div
          ...
          style={{
            background: isActive 
              ? `radial-gradient(circle at top left, color-mix(in srgb, var(--accent-on) 40%, transparent), transparent)`
              : `radial-gradient(circle at top left, color-mix(in srgb, var(--accent-on) 80%, transparent), transparent)`
          }}
        />
        ...
        {isActive && (
          <motion.div 
            layoutId="glow" 
            className="absolute inset-0 pointer-events-none z-0" style={{ background: 'color-mix(in srgb, var(--accent-on) 10%, transparent)' }} 
