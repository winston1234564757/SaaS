...
export function SupportChatPage({ user, userRole, initialTicketId }: SupportChatPageProps) {
  ...
  const uploadFile = async (ticketId: string): Promise<string | null> => {
    ...
    if (uploadError) {
      console.error('[SupportChatPage] Upload error:', uploadError.message);
      return null;
    }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ticketId) {
        ...
        if (selectedFile) {
          fileUrl = await uploadFile(ticketId);
          removeFile();
        }

        // If we created a new ticket, the first message is already sent inside createSupportTicketAction!
        // We only send another message if we had an existing ticket OR if we have an attachment to send
        if (activeTicketId || fileUrl) {
          const { error, messageData } = await sendSupportMessageAction(
            ticketId,
            currentMsg.trim() || 'Файл прикріплено',
            fileUrl
          );
          if (error) throw new Error(error);
          if (messageData) {
  ...
  const handleBack = () => {
    if (window.history.length > 1) {
      ...
      router.push(userRole === 'master' ? '/dashboard/support' : '/my/bookings');
    }
  };
    ...
    <div className="flex flex-col h-dvh w-full overflow-hidden bg-transparent text-[var(--text-primary)]">
            ...
            <div className="w-full space-y-2 pt-4">
              ...
              <div className="flex flex-col gap-2">
                {SUGGESTIONS.map((text) => (
                  <button
                    key={text}
                    onClick={() => handleSuggestionClick(text.replace(/ [^\s]+$/, ''))}
                ...
                >
                  <div
                    className={`max-w-[80%] sm:max-w-[70%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${
                      isOwn
                        ? 'bg-[var(--accent)] text-[var(--accent-on)] rounded-br-none'
                        : 'bg-[var(--surface)] text-[var(--text-primary)] border border-[var(--border)] rounded-bl-none backdrop-blur-md'
                    }`}
                  >
                      ...
                      <div className="mt-2 overflow-hidden rounded-xl border border-[var(--border)] shadow-inner">
                        <a href={msg.attachment_url} target="_blank" rel="noopener noreferrer">
                          <img
                            ...
                            alt="Вкладення"
                            className="max-h-60 w-full object-cover transition hover:scale-102 cursor-zoom-in"
                          />
                        </a>
                      </div>
                    )}
      ...
      <div className="shrink-0 bg-[var(--surface)] border-t border-[var(--border)] p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] backdrop-blur-xl z-10">
        <div className="max-w-2xl mx-auto w-full space-y-3">
          {/* Suggestion Chips strip above input when ticket has messages */}
          {messages.length > 0 && (
