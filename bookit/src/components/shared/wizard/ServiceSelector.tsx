              ...
              {services.filter(s => s.category === cat).map(svc => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-start gap-3 px-4 pt-4 pb-4 rounded-xl border text-left w-full transition-all cursor-pointer ${
                      sel
                        ? 'bg-primary/10 border-primary/40 shadow-sm'
                        : 'bg-secondary border-border hover:border-primary/25 hover:bg-secondary/90'
                    } active:scale-[0.95]`}
                  >
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 bg-primary/10 border border-primary/20"
                    >
                      <ServiceIcon name={svc.icon_name} size={16} className="text-primary" />
                    </div>
                    ...
                    <div className="flex items-start gap-2 flex-shrink-0 mt-0.5">
                      ...
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                        sel ? 'bg-primary border-primary' : 'border-border bg-secondary/60'
                      }`}>
            ...
            <div className="flex gap-3 overflow-x-auto pb-4 -mx-1 px-1 scrollbar-hide">
              {partners.map(p => (
                <Link
                  key={p.id}
                  href={`/${p.slug}`}
                  className="flex-shrink-0 w-[140px] bento-card p-3 flex flex-col items-center text-center gap-2 hover:bg-secondary/90 active:scale-[0.95] transition-transform duration-100 cursor-pointer scroll-ml-1"
                >
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-24 px-3 py-2 rounded-xl bg-secondary border border-border text-sm text-foreground placeholder-muted-foreground outline-none focus:bg-secondary focus:border-primary focus:ring-2 focus:ring-primary/20"
                  />
