...
import {
  ...
  type TimeRange, type SlotReason,
...
export function useBookingScheduleData({
  ...
  // +30 days from today matches FreeSlotsWidget/StoryGenerator → same TanStack cache key
  ...
  // ── Schedule-derived: slots for selected date (via shared engine) ─────────────
  const availableTimes = useSlotsFromStore(
    selectedDate ? toISO(selectedDate) : null,
    effectiveDuration,
    workingHours?.buffer_time_minutes ?? 0,
    workingHours ?? null,
    scheduleStore,
  );
  ...
  const slots = useMemo(() => {
    ...
    return scoreSlots(
      availableTimes.map(time => ({ time, available: true, reason: 'available' as SlotReason })),
      { clientHistoryTimes },
    );
  }, [availableTimes, clientHistoryTimes]);
