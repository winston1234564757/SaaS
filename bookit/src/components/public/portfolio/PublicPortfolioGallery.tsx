      ...
      <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 snap-x snap-mandatory scrollbar-none">
        {items.slice(0, 2).map((item, i) => (
          ...
          >
            <Link href={`/${masterSlug}/portfolio/${item.id}`}>
              <div
                className="w-44 rounded-xl overflow-hidden transition-transform active:scale-[0.97]"
        ...
        <div className="shrink-0 snap-start w-44">
          <Link href={`/${masterSlug}/portfolio`}>
            <div
              className="w-44 h-full min-h-[220px] rounded-xl flex flex-col items-center justify-center gap-2 transition-colors active:scale-[0.97]"
