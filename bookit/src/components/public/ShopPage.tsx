  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 pb-32 flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3 pt-2">
        <Link
          href={`/${masterSlug}`}
          className="w-9 h-9 rounded-2xl bg-white/70 border border-white/70 flex items-center justify-center text-[#6B5750] hover:bg-white transition-all shrink-0"
        >
          <ArrowLeft size={18} />
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="heading-serif text-xl text-[#2C1A14] leading-tight">Магазин</h1>
        ...
        {cartCount > 0 && (
          ...
          >
            <button
              ...
              className="pointer-events-auto w-full max-w-lg mx-auto flex items-center justify-between px-5 py-4 rounded-2xl bg-[#2C1A14] text-white shadow-xl active:scale-[0.98] transition-all"
            >
              <div className="flex items-center gap-2.5">
                <div className="relative">
                  <ShoppingBag size={20} />
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-[#789A99] text-[9px] font-bold flex items-center justify-center">
                    {cartCount}
                  </span>
                </div>
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[4/3] overflow-hidden bg-[#F5EDE8]">
        ...
        {qty > 0 && (
          <div className="absolute top-2 right-2 min-w-[22px] h-[22px] px-1.5 rounded-full bg-[#789A99] flex items-center justify-center">
            <span className="text-[10px] font-black text-white">{qty}</span>
          </div>
        )}
        {p.stock_qty > 0 && p.stock_qty <= 5 && qty === 0 && (
          <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded-full bg-[#D4935A]/90 backdrop-blur-sm">
            <span className="text-[10px] font-bold text-white">Залишок: {p.stock_qty}</span>
          </div>
        )}
      </div>
      <div className="flex flex-col gap-1 p-3">
        <p className="text-xs font-bold text-[#2C1A14] leading-tight line-clamp-2">{p.name}</p>
        {p.description && <p className="text-[10px] text-[#A8928D] line-clamp-1">{p.description}</p>}
        <p className="text-base font-black text-[#2C1A14] mt-0.5">{(p.price_kopecks / 100).toFixed(0)} ₴</p>
      </div>
    </motion.button>
  );
}
      ...
      {p && (
        <>
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-black/10 backdrop-blur-sm flex items-center justify-center"
            >
              <X size={16} className="text-[#2C1A14]" />
            </button>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {photos.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 shadow-lg flex items-center justify-center text-[#2C1A14] opacity-0 group-hover:opacity-100 transition-all hover:scale-110 active:scale-90 z-20 hidden md:flex"
                    >
                      <ChevronLeft size={24} />
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); setPhotoIdx(p => (p + 1) % photos.length); }}
                      className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 shadow-lg flex items-center justify-center text-[#2C1A14] opacity-0 group-hover:opacity-100 transition-all hover:scale-110 active:scale-90 z-20 hidden md:flex"
                    >
              ...
              <div className="px-5 pt-4 pb-28 flex flex-col gap-4">
                <div>
                  <h2 className="text-xl font-black text-[#2C1A14] leading-tight">{p.name}</h2>
                  <p className="text-2xl font-black text-[#2C1A14] mt-2">{price} ₴</p>
                  {p.stock_qty > 0 && p.stock_qty <= 10 && (
                    <p className="text-xs text-[#D4935A] font-semibold mt-1">⚡ Залишилось {p.stock_qty} шт</p>
                  )}
                  {p.stock_qty <= 0 && (
                    <p className="text-xs text-[#C05B5B] font-semibold mt-1">Немає в наявності</p>
                  )}
                </div>

                {p.description && (
                  <div>
                    <p className="text-xs font-bold text-[#A8928D] uppercase tracking-wider mb-1.5">Про товар</p>
                    <p className="text-sm text-[#6B5750] leading-relaxed">{p.description}</p>
                  </div>
                )}
                    ...
                    {!inCart ? (
                      <>
                        ...
                        <div className="flex items-center gap-3">
                          <p className="text-xs font-semibold text-[#6B5750]">Кількість</p>
                          <div className="flex items-center rounded-2xl overflow-hidden border" style={{ background: '#F5E8E3', borderColor: 'rgba(44,26,20,0.10)' }}>
                            <button onClick={() => setLocalQty(q => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center text-[#6B5750] active:bg-[#E8D5CF] transition-colors">
                              <Minus size={15} />
                            </button>
                            <span className="w-8 text-center font-black text-[#2C1A14]">{localQty}</span>
                            <button onClick={() => setLocalQty(q => Math.min(p.stock_qty, q + 1))} disabled={localQty >= p.stock_qty} className="w-10 h-10 flex items-center justify-center text-[#2C1A14] active:bg-[#E8D5CF] transition-colors disabled:opacity-30">
                              <Plus size={15} />
                            </button>
                          </div>
                          <p className="text-xs text-[#A8928D] ml-auto">{((p.price_kopecks * localQty) / 100).toFixed(0)} ₴</p>
                        </div>
                        <motion.button
                          onClick={handleAddToCart}
                          whileTap={{ scale: 0.97 }}
                          className="w-full py-4 rounded-2xl bg-[#2C1A14] text-white font-bold text-base flex items-center justify-center gap-2"
                      ...
                      <>
                        <p className="text-xs font-semibold text-[#5C9E7A] flex items-center gap-1.5">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex items-center rounded-2xl overflow-hidden border flex-1" style={{ background: '#F5E8E3', borderColor: 'rgba(44,26,20,0.10)' }}>
                            <button onClick={() => onQtyChange(qty - 1)} className="w-10 h-10 flex items-center justify-center text-[#6B5750] active:bg-[#E8D5CF] transition-colors">
                              <Minus size={15} />
                            </button>
                            <span className="flex-1 text-center font-black text-[#2C1A14]">{qty}</span>
                            <button onClick={() => qty < p.stock_qty ? onQtyChange(qty + 1) : undefined} disabled={qty >= p.stock_qty} className="w-10 h-10 flex items-center justify-center text-[#2C1A14] active:bg-[#E8D5CF] transition-colors disabled:opacity-30">
                              <Plus size={15} />
                            </button>
                          </div>
                          <button onClick={() => { onQtyChange(0); onClose(); }} className="px-4 py-3 rounded-2xl border border-[#2C1A14]/20 text-[#2C1A14] text-sm font-semibold">
                            Видалити
                          </button>
                        </div>
                      </>
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 shrink-0">
              <h2 className="text-base font-bold text-[#2C1A14]">Оформлення замовлення</h2>
              <button onClick={onClose} className="w-8 h-8 rounded-full bg-[#F5E8E3] flex items-center justify-center text-[#6B5750]">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-28 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                <p className="text-[10px] font-bold text-[#A8928D] uppercase tracking-wider">Товари</p>
                {cart.map(item => (
                  <div key={item.product.id} className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#F5E8E3] shrink-0">
                      {item.product.photos[0] && <Image src={item.product.photos[0]} alt="" width={40} height={40} className="object-cover w-full h-full" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-[#2C1A14] truncate">{item.product.name}</p>
                      <p className="text-[10px] text-[#A8928D]">{item.qty} шт × {(item.product.price_kopecks / 100).toFixed(0)} ₴</p>
                    </div>
                    <p className="text-xs font-bold text-[#2C1A14]">
              ...
              {!isAuth && (
                <div className="flex flex-col gap-3">
                  <p className="text-[10px] font-bold text-[#A8928D] uppercase tracking-wider">Ваші контакти</p>
                  <div className="flex flex-col gap-2">
                    <div className="relative">
                      <User size={16} className="absolute left-4 top-3.5 text-[#A8928D]" />
                      <input
                        type="text"
                        placeholder="Ваше ім'я"
                        value={name}
                        onChange={e => setName(e.target.value)}
                        className="w-full pl-11 pr-4 py-3 rounded-2xl bg-white border border-white/80 text-sm text-[#2C1A14] outline-none focus:border-[#789A99]/50 shadow-sm"
                      />
                    </div>
                    <div className="relative">
                      <Phone size={16} className="absolute left-4 top-3.5 text-[#A8928D]" />
                      <input
                        type="tel"
              ...
              {/* Delivery / Pickup */}
              <div className="flex flex-col gap-3">
                <p className="text-[10px] font-bold text-[#A8928D] uppercase tracking-wider">Спосіб отримання</p>
                ...
                {delivery === 'pickup' ? (
                  <div className="flex flex-col gap-3">
                    <p className="text-[11px] text-[#6B5750] flex items-center gap-1.5 px-1">
                      <CalendarDays size={14} className="text-[#789A99]" /> Оберіть зручний день
                    </p>
                    ...
                    <div className="relative group">
                      ...
                      <button 
                        ...
                        className="absolute -left-2 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full bg-white/90 shadow-md flex items-center justify-center text-[#2C1A14] opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <ChevronLeft size={16} />
                      </button>
                      <button 
                        onClick={() => { const el = document.getElementById('day-slider'); el?.scrollBy({ left: 120, behavior: 'smooth' }); }}
                        className="absolute -right-2 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full bg-white/90 shadow-md flex items-center justify-center text-[#2C1A14] opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        ...
                        {nextDays.map(d => (
                          <button
                            ...
                            className={cn(
                              ...
                              pickupDate === d.date
                                ? 'bg-[#2C1A14] border-[#2C1A14] text-white shadow-lg shadow-[#2C1A14]/20 scale-[1.02]'
                                : d.isWorking
                                  ? 'bg-white border-white/80 text-[#2C1A14] hover:border-[#789A99]'
                                  : 'bg-[#F5E8E3]/30 border-transparent text-[#A8928D] opacity-50 cursor-not-allowed'
                            )}
                          >
                        ...
                        {nextDays.map((d, i) => (
                          <div 
                            ...
                            className={cn(
                              ...
                              pickupDate === d.date ? "bg-[#2C1A14] w-3" : "bg-[#D4B9B0]/40"
                            )} 
                          />
                        ))}
                      </div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-2xl bg-white border border-white/80 text-sm text-[#2C1A14] outline-none focus:border-[#789A99]/50 shadow-sm"
                  />
                )}
              </div>
              ...
              <div className="mt-auto pt-4 flex flex-col gap-4">
                <div className="flex items-center justify-between px-1">
                  <span className="text-sm font-medium text-[#6B5750]">До сплати</span>
                  <span className="text-2xl font-black text-[#2C1A14]">{(total / 100).toFixed(0)} ₴</span>
                </div>

                {error && <p className="text-xs text-[#C05B5B] bg-[#C05B5B]/5 py-2 px-3 rounded-xl border border-[#C05B5B]/10">{error}</p>}

                <button
                  onClick={handleOrder}
                  disabled={isPending || cart.length === 0}
                  className="w-full py-4 rounded-2xl bg-[#2C1A14] text-white font-bold text-base flex items-center justify-center gap-2 disabled:opacity-50 active:scale-[0.98] transition-all shadow-lg shadow-[#2C1A14]/20"
                >
                  {isPending
                ...
                <p className="text-[10px] text-[#A8928D] text-center">
...
function OrderSuccess({ masterSlug, masterName }: { masterSlug: string; masterName: string }) {
  return (
    <div className="max-w-lg mx-auto px-4 py-24 flex flex-col items-center gap-5 text-center">
      ...
      >
        <CheckCircle2 size={40} className="text-[#5C9E7A]" />
      </motion.div>
      <div>
        <h2 className="heading-serif text-2xl text-[#2C1A14] mb-2">Замовлення прийнято!</h2>
      ...
      <Link
        href={`/${masterSlug}`}
        className="mt-2 inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-[#789A99] text-white text-sm font-semibold hover:bg-[#5C7E7D] transition-colors shadow-lg shadow-[#789A99]/20"
      >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-[#2C1A14] text-white' : 'bg-white/70 text-[#6B5750] border border-white/70 hover:bg-white'
      )}
    >
      {children}
    </button>
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : 'bg-white/60 text-[#6B5750] border-white/80 hover:bg-white/80'
      )}
    >
      {icon} {label}
    </button>
...
function EmptyShop({ masterSlug }: { masterSlug: string }) {
  return (
    <div className="col-span-2 bento-card p-10 flex flex-col items-center gap-3 text-center">
      <Package size={32} className="text-[#A8928D]" />
      <p className="text-sm font-semibold text-[#2C1A14]">Товарів поки немає</p>
      <Link href={`/${masterSlug}`} className="text-xs text-[#789A99] hover:underline flex items-center gap-1">
