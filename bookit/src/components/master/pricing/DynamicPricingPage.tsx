...
// Shared input class
// ─────────────────────────────────────────────────────────────────────────────
function numInput(focusColor: string) {
  return `w-full px-3 py-2.5 rounded-xl border border-[#E8D5CF] bg-white/60 text-sm text-[#2C1A14]
          focus:outline-none focus:ring-2 focus:border-current transition-colors`
    + ` focus:ring-[${focusColor}]/30 focus:border-[${focusColor}]/50`;
}
...
export function DynamicPricingPage({ initial }: Props) {
  ...
  async function handleSave() {
    ...
    if (result.error) { setSaveError(result.error); setSaved(false); }
    else              { setSaved(true); }
    setSaving(false);
  }
