...
export function NavigationStrip() {
  ...
  return (
    <div className="sticky top-4 z-[100] px-4 w-full max-w-2xl mx-auto group/nav">
      <div className="relative h-14 w-full bg-white/40 backdrop-blur-xl border border-white/60 rounded-full shadow-lg shadow-black/5 overflow-hidden">
        {/* Mobile Left Hint */}
        <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-white/40 to-transparent z-10 pointer-events-none lg:hidden" />
        ...
        <div className="flex items-center gap-6 h-full overflow-x-auto scrollbar-hide px-6 justify-start lg:justify-center">
          {ITEMS.map((item) => (
            <button
              ...
              className={cn(
                "relative text-[11px] font-bold uppercase tracking-widest whitespace-nowrap transition-colors py-1 shrink-0",
        ...
        {/* Mobile Right Hint */}
        <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-white/40 to-transparent z-10 pointer-events-none lg:hidden" />
      </div>
    </div>
  );
}
