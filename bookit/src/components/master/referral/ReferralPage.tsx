    ...
    <div className="flex flex-col gap-4 pb-8">
      ...
      {!isDrawer && (
        <div className={cn(
          ...
          <p className="text-sm text-[#A8928D]">Запрошуй колег — отримуй знижки на Pro</p>
        </div>
      )}
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <p className="text-sm font-semibold text-[#2C1A14]">Твоє реферальне посилання</p>
            <p className="text-xs text-[#A8928D]">Запрошений отримує 14 днів Pro trial</p>
          </div>
        </div>
      ...
      >
        {[
          { label: 'Запрошено', value: referralCount,                  icon: <Users size={15} className="text-[#6B5750]" /> },
          { label: 'Активних',  value: activeReferralCount,            icon: <Check size={15} className="text-[#5C9E7A]" /> },
          { label: 'Запас %',   value: `${reservePct}%`,              icon: <Zap size={15} className="text-[#D4935A]" /> },
        ].map(({ label, value, icon }) => (
          <div key={label} className="bento-card p-4 flex flex-col items-center gap-1 shadow-sm">
            {icon}
            <p className="text-2xl font-bold text-[#2C1A14] leading-none mt-1">{value}</p>
            <p className="text-[9px] font-semibold text-[#A8928D] uppercase tracking-wider text-center pt-0.5">{label}</p>
          </div>
        ))}
      </motion.div>
      ...
      <motion.div
        ...
        className="flex gap-1 p-1 bg-[#F5E8E3]/60 rounded-2xl"
      >
        ...
        {tab === 'overview' ? (
          ...
          >
            ...
            <div className={cn(
              ...
              reservePct > 0
                ? 'bg-[#D4935A]/5 border-[#D4935A]/25'
                : 'border-[#E8D0C8]'
            )}>
              <div className="flex items-center gap-2.5 mb-3">
                <div className={cn(
                  'w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0',
                  reservePct > 0 ? 'bg-[#D4935A]/15' : 'bg-[#E8D0C8]/60'
                )}>
                  <Zap size={16} className={reservePct > 0 ? 'text-[#D4935A]' : 'text-[#C8B0A8]'} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#2C1A14]">Банк знижок</p>
                  <p className="text-xs text-[#A8928D]">Bounties (+10% за першу оплату партнера) + залишок з минулого місяця</p>
                </div>
              </div>
              ...
              {reservePct > 0 ? (
                <div className="flex items-center justify-between mt-1">
                  <div>
                    <p className="text-2xl font-bold text-[#D4935A]">−{reservePct}%</p>
                    <p className="text-xs text-[#A8928D] mt-0.5">
                      Накопичено у вашому резерві
                    </p>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-[#D4935A]/10 flex items-center justify-center text-lg flex-shrink-0">
                    🏦
                  </div>
                </div>
              ) : (
            ...
            <div className="bento-card p-5">
              <div className="flex items-center gap-2.5 mb-3">
                <div className={cn(
                  'w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0',
                  currentLTDisc > 0 ? 'bg-[#789A99]/15' : 'bg-[#E8D0C8]/60'
                )}>
                  <Crown size={16} className={currentLTDisc > 0 ? 'text-[#789A99]' : 'text-[#C8B0A8]'} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#2C1A14]">Твій постійний статус</p>
                  <p className="text-xs text-[#A8928D]">Залежить від кількості активних партнерів</p>
                </div>
              </div>

              {currentLTDisc > 0 && (
                <div className="mb-3 flex items-center gap-2">
                  <span className="text-2xl font-bold text-[#789A99]">−{lifetimePct}%</span>
                  <span className="text-xs text-[#A8928D]">постійно за {activeReferralCount} активних</span>
                </div>
              )}

              {atHardCap ? (
                <div className="flex items-center gap-2 text-sm text-[#5C9E7A] font-semibold">
                  <Check size={15} />
                  Максимальний статус — −50% назавжди
                </div>
              ) : tierProgress ? (
                <>
                  <div className="flex justify-between text-xs text-[#A8928D] mb-2">
                    <span>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        reached
                          ? 'border-[#789A99]/35 bg-[#789A99]/8'
                          : 'border-[#E8D0C8] bg-white/40'
                      )}
                    >
                      {reached
                        ? <Check size={11} className="text-[#789A99]" />
                        : <Lock  size={11} className="text-[#C8B0A8]" />
                      }
                      <p className={cn(
                        'text-[10px] font-bold leading-tight',
                        reached ? 'text-[#789A99]' : 'text-[#A8928D]'
                      )}>
                        {tier.pct}
                      </p>
            ...
            {isPro && (
              decision.shouldGrantFree ? (
                /* Branch A — free month */
                <div className="bento-card p-5 bg-[#5C9E7A]/8 border-2 border-[#5C9E7A]/30">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-lg">🎉</span>
                    <p className="text-sm font-bold text-[#5C9E7A]">Наступний місяць безкоштовний!</p>
                  </div>
                  ...
                  {newReservePct > 0 && (
                    <div className="flex items-center justify-between text-xs pt-2 border-t border-[#5C9E7A]/20">
                      <span className="text-[#A8928D]">Залишок буде збережено у запасі</span>
                      <span className="font-bold text-[#789A99]">+{newReservePct}%</span>
                    </div>
                  )}
                  {/* Breakdown */}
                  <div className="mt-3 flex flex-col gap-1 pt-2 border-t border-[#5C9E7A]/15">
                    {lifetimePct > 0 && (
                      <div className="flex justify-between text-xs">
                        <span className="text-[#A8928D]">Статус Альянсу</span>
                        <span className="text-[#789A99] font-semibold">+{lifetimePct}%</span>
                      </div>
                    )}
                    {reservePct > 0 && (
                      <div className="flex justify-between text-xs">
                        <span className="text-[#A8928D]">Банк знижок (bounties + залишок)</span>
                        <span className="text-[#D4935A] font-semibold">+{reservePct}%</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className={cn(
                  'bento-card p-5 border',
                  discountPct > 0 ? 'bg-[#5C9E7A]/5 border-[#5C9E7A]/20' : 'border-[#E8D0C8]'
                )}>
                  <p className="text-xs font-bold text-[#6B5750] uppercase tracking-widest mb-3 opacity-60">
                    Наступний платіж
                  </p>
                  ...
                  {discountPct > 0 && discountPct < 100 && (
                    <div className="mb-3">
                      <div className="flex justify-between text-xs text-[#A8928D] mb-1">
                        <span>До безкоштовного місяця</span>
                        <span className="font-semibold text-[#2C1A14]">{discountPct}%</span>
                      </div>
                  ...
                  <div className="flex items-end gap-2 mb-2">
                    <p className="text-3xl font-bold text-[#2C1A14]">{nextMonthUah} ₴</p>
                    {discountPct > 0 && (
                      <>
                        <p className="text-sm text-[#A8928D] line-through mb-1">700 ₴</p>
                        <span className="text-xs font-bold text-[#5C9E7A] bg-[#5C9E7A]/10 px-2 py-0.5 rounded-full mb-1">
                          -{discountPct}%
                        </span>
                      </>
                    )}
            ...
            {isPro && trialLeft !== null && trialLeft > 0 && (
              <div className="bento-card p-4 flex items-center gap-3 border border-[#D4935A]/20 bg-[#D4935A]/5">
                <Clock size={16} className="text-[#D4935A] flex-shrink-0" />
                <p className="text-xs text-[#6B5750]">
                  Pro trial: залишилось{' '}
                  <span className="font-semibold text-[#2C1A14]">
                    {pluralize(trialLeft, ['день', 'дні', 'днів'])}
                  </span>
                </p>
              </div>
            )}
            ...
            {/* Hard cap notice */}
            <div className="flex items-start gap-2 text-xs text-[#A8928D] px-1">
          ...
          >
            {history.length === 0 ? (
              <div className="bento-card p-8 flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-sm font-semibold text-[#6B5750]">Ще нікого немає</p>
              ...
              history.map((item) => (
                ...
                >
                  <div className={cn(
                    'w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0',
                    item.isFirstPaymentMade ? 'bg-[#5C9E7A]/12' : 'bg-[#E8D0C8]/60'
                  )}>
                    {item.isFirstPaymentMade
                      ? <Zap size={15} className="text-[#5C9E7A]" />
                      : <Clock size={15} className="text-[#C8B0A8]" />
                    }
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#2C1A14] truncate">{item.refereeName}</p>
                    <p className="text-xs text-[#A8928D]">
                  ...
                  <div className="text-right flex-shrink-0">
                    {item.isFirstPaymentMade ? (
                      <span className="text-xs font-semibold text-[#5C9E7A] bg-[#5C9E7A]/10 px-2 py-0.5 rounded-full">
                        Bounty нараховано
                      </span>
                    ) : (
                      <span className="text-xs text-[#A8928D] bg-[#F5E8E3]/80 px-2 py-0.5 rounded-full">
                        Trial
                      </span>
                    )}
                  </div>
