  ...
  if (isLoading) {
    return (
      <div>
        ...
        <div className="skeleton-shimmer rounded-lg" style={{ height: 180 }} />
      </div>
    );
  }
    ...
    <>
      <AnimatePresence>
        {tooltipInfo && (
          ...
          >
            <div
              className="px-3 py-2 rounded-lg text-[12px] font-semibold whitespace-nowrap shadow-lg"
      ...
      <div>
          ...
          <div className="flex flex-col gap-[3px] pt-[18px] pr-1 shrink-0">
            {HOURS.map(h => (
              <div key={h} className="h-[17px] flex items-center">
                <span className="font-mono text-[8px] text-[var(--text-tertiary)] opacity-40 w-5 text-right">
                  {h}
                </span>
              </div>
            ))}
          ...
          <div className="flex gap-[3px] flex-1">
            {DAYS.map((day, dIdx) => (
              <div key={day} className="flex flex-col gap-[3px] flex-1 min-w-0">
                <div className="font-mono text-[9px] font-bold text-[var(--text-tertiary)] text-center leading-none mb-[2px] opacity-60">
                  {day}
                </div>
                {HOURS.map((_, hIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className="h-[17px] cursor-pointer bg-transparent border-0 p-0 w-full"
                      style={{
                        borderRadius: '3px',
                        background:   'var(--accent)',
                        opacity:      intensity === 0 ? 0.08 : 0.12 + intensity * 0.88,
        ...
        {max === 0 && (
          <p className="text-[11px] tracking-[0.08em] uppercase text-[var(--text-tertiary)] mt-3 opacity-40">
