...
function CircleProgress({ progress }: { progress: number }) {
  ...
  const color =
    ...
    progress >= 40 ? 'var(--accent)' :
    'var(--sage)';

  return (
    <svg width={86} height={86} viewBox="0 0 86 86" className="shrink-0 -rotate-90">
      <circle cx={43} cy={43} r={RADIUS} fill="none" stroke="var(--border)" strokeWidth={STROKE} />
      <motion.circle
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          style={{ boxShadow: '0 4px 24px color-mix(in srgb, var(--sage) 10%, transparent)' }}
        >
          {celebrating ? (
              ...
              >
                {incomplete.map(step => (
                  <motion.div key={step.key} variants={listItem}>
                    <Link href={step.href}>
                      <motion.div
                        whileHover={{ x: 3, backgroundColor: 'color-mix(in srgb, var(--accent-on) 70%, transparent)' }}
                        whileTap={{ scale: 0.98 }}
