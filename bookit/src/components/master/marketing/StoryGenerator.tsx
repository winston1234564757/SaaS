                ...
                <div className="flex flex-wrap gap-2">
                  {flashWinSlots.map(s => (
                    <button
                      ...
                      className={cn(
                        "px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer active:scale-[0.88]",
                        flashWinTime === s
                          ? "bg-[var(--btn-primary-bg)] text-[var(--accent-on)] shadow-[0_2px_8px_rgba(120,154,153,0.35)]"
        ...
        {MODES.map(m => {
          ...
          return (
            <button key={m.id} type="button" onClick={() => setMode(m.id)}
              className={cn(
                "relative flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer shrink-0 active:scale-[0.88]",
                active
                  ? "bg-[var(--btn-primary-bg)] text-[var(--accent-on)] shadow-[0_4px_12px_rgba(120,154,153,0.35)]"
                  : "bg-secondary/70 text-text-secondary border border-border"
              )}
            >
              ...
              {m.premium && (
                <span className={cn(
                  "ml-0.5 text-[9px] font-bold px-1 py-0.5 rounded-md",
                  active ? "bg-accent-on/25 text-accent-on" : "bg-warning/15 text-warning"
                )}>
                  PRO
                </span>
              )}
          ...
          <div className="pt-4 border-t border-border space-y-4">
                  ...
                  <div className="flex bg-secondary/40 rounded-xl p-0.5 border border-border">
                    {(['top', 'center', 'bottom'] as const).map(p => (
                      <button key={p} type="button" onClick={() => { setPlatePos(p); onControlChange(); }}
                        className={`flex-1 py-1 text-[10px] font-bold rounded-lg transition-all active:scale-[0.88] cursor-pointer ${platePos === p ? 'bg-surface shadow-sm text-foreground' : 'text-muted-foreground/60 hover:text-muted-foreground'}`}>
            ...
            <div>
              ...
              <input
                ...
                className="w-full cursor-pointer h-1.5 bg-secondary/50 rounded-lg appearance-none"
