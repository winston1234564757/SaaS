...
export async function getMasterClients(masterId: string) {
  const admin = createAdminClient();

  // RPC aggregates all clients from bookings (not just client_master_relations)
  const { data } = await admin.rpc('get_master_clients', { p_master_id: masterId });
...
export async function createPortfolioDraft(): Promise<string> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect('/login');
  ...
  if (error || !item) throw new Error('draft_creation_failed');

  // No revalidatePath here — draft is unpublished and invisible on the list.
  // The list cache is invalidated client-side after navigation back.
  return item.id;
}
...
export async function updatePortfolioItem(itemId: string, formData: FormData) {
  ...
  // Guard: if publishing, enforce Starter tier limit
  if (parsed.data.is_published) {
    ...
    // Only check limit if transitioning from unpublished → published
    if (current && !current.is_published) {
      ...
      if ((mp?.subscription_tier ?? 'starter') === 'starter') {
        const { count } = await admin
          .from('portfolio_items')
          .select('id', { count: 'exact', head: true })
          .eq('master_id', user.id)
...
export async function togglePublishPortfolioItem(itemId: string, isPublished: boolean) {
  ...
  // Guard: enforce Starter tier limit when publishing
  if (isPublished) {
    const { data: mp } = await admin
      .from('master_profiles')
      .select('subscription_tier')
